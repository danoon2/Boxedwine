...
enum NativeSupport {
    ...
    static let privacyURL = URL(string: "https://www.boxedwine.org/privacy.html")!
...
private struct NativeAboutView: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            Link("Privacy Policy", destination: NativeSupport.privacyURL)
...
enum NativeLibraryCommand: Hashable {
    case add, restore, open, stop, settings, troubleshooting, chooseProgram, runAnotherProgram, backup, remove, log
    case find, allApps, recent, demos, removed, recovery, notepad, minesweeper
}
...
struct NativeLibraryCommands: Commands {
    ...
    var body: some Commands {
        CommandGroup(replacing: .appInfo) {
            ...
            Button("Support Boxedwine…") { openWindow(id: "support-boxedwine") }
#endif
        }
        CommandGroup(replacing: .newItem) {
            ...
            action("Troubleshooting…", .troubleshooting)
        ...
        CommandGroup(replacing: .help) {
            ...
            Link("Report an Issue on GitHub…", destination: NativeSupport.issuesURL)
            Divider()
            action("Play Minesweeper", .minesweeper)
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                topic("Play Minesweeper", symbol: "gamecontroller") {
                    Text("Choose Play Minesweeper in the sidebar or Help menu. Wine’s Minesweeper opens and is added to All Apps. Reveal the safe squares and mark suspected mines with a secondary click.")
                }
                #if !BOXEDWINE_APP_STORE
                ...
                topic("Add an app you already have", symbol: "plus.app") {
                    ...
                    Text("Boxedwine copies those files into your library. After an installer closes, choose the program you want to open and save it to the library. If setup did not finish, open Troubleshooting and use Run Installer Again.")
                }
                topic("Keep your files and saves", symbol: "archivebox") {
                    Text("Back Up App in App Settings saves an app’s Windows files, settings and Wine package together. Close the app first. If you’ve edited its settings, Save and Back Up includes those changes. Restore App Backup creates a separate library entry.")
...
#if BOXEDWINE_APP_STORE
                    shortcut("All Apps / Recently Opened", "⌘1 / ⌘2")
#else
