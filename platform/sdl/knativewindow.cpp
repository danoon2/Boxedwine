...
public:
    ...
    // from KNativeWindow
    void screenChanged(U32 width, U32 height, U32 bpp) override {
        this->width = width;
        this->height = height;
        if (this->bpp == bpp) {
            screenResized();
    ...
    void clear() override;
    void putBitsOnWnd(const WndPtr& w, U8* bits, U32 bitsPerPixel, U32 srcPitch, S32 dstX, S32 dstY, U32 width, U32 height, U32* palette, bool isDirty) override;
    ...
    void drawWnd(const WndPtr& w, U8* bytes, U32 pitch, U32 bpp, U32 width, U32 height) override;
...
void KNativeWindowSdl::destroyScreen() {
...
void KNativeWindowSdl::screenResized() {
    BOXEDWINE_CRITICAL_SECTION_WITH_MUTEX(sdlMutex);
    DISPATCH_MAIN_THREAD_BLOCK_THIS_BEGIN
        if (contextCount) {
            int cx = screenWidth();
...
void KNativeWindowSdl::displayChanged() {    
    firstWindowCreated = 1;
    if (contextCount) {
...
void KNativeWindowSdl::putBitsOnWnd(const WndPtr& w, U8* bits, U32 bitsPerPixel, U32 srcPitch, S32 dstX, S32 dstY, U32 width, U32 height, U32* palette, bool isDirty) {
    preDrawWindow();
    ...
    if (isDirty && bitsPerPixel == 8) {
        for (U32 y = 0; y < height; y++) {
            ...
            U32* dstLine = (U32*)(sdlBuffer + dstPitch * y);
...
void KNativeWindowSdl::drawWnd(const WndPtr& w, U8* bytes, U32 pitch, U32 bpp, U32 width, U32 height) {
    if (!firstWindowCreated) {
        BOXEDWINE_CRITICAL_SECTION_WITH_MUTEX(sdlMutex);
        DISPATCH_MAIN_THREAD_BLOCK_THIS_BEGIN
            displayChanged();
