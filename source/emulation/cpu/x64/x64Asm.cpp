...
#define CPU_OFFSET_HOST_FLAGS (U32)(offsetof(x64CPU, currentHostFlags))
...
bool writesFlags[InstructionCount];
bool writesFlagsInitialized = false;
...
X64Asm::X64Asm(x64CPU* cpu) : X64Data(cpu), tmp1InUse(false), tmp2InUse(false), tmp3InUse(false), tmp4InUse(false), param1InUse(false), param2InUse(false), param3InUse(false), param4InUse(false)  {
    if (!writesFlagsInitialized) {
        writesFlagsInitialized = true;
        for (int i = Movsb; i <= Scasd; i++) {
            writesFlags[i] = true;
        }
        // x64Ops_fpu/PushPopFlags will add to writesFlags
    }
}
...
U8 X64Asm::getHostMem(U8 regEmulatedAddress, bool isRex) {
    ...
    U32 disp = 0;
    if (instructionInfo[this->currentOp->inst].writeMemWidth) {
        disp = K_NUMBER_OF_PAGES * 8;
    }
    writeToRegFromMem(resultReg, true, HOST_MEM_READ, true, resultReg, true, 3, disp, 8, false); // shift page << 3 (page*8), since sizeof(U64)==8 to get the value in memOffsets[page]
...
void X64Asm::shiftLeftRegReg(U8 reg, bool isRegRex, U8 regAmount, bool isRegAmountRex, bool is64bit) {
    if (x64CPU::hasBMI2 && isRegRex && isRegAmountRex) {
        bmi2ShiftLeftReg(reg, reg, isRegRex, regAmount);
    } else {
        pushNativeReg(1, true);
        writeToRegFromReg(1, false, regAmount, isRegAmountRex, 4);
        if (isRegRex || is64bit) {
            write8(REX_BASE | (isRegRex ? REX_MOD_RM : 0) | (is64bit ? REX_64 : 0));
        }
        write8(0xd3);
        write8(0xe0 | reg);
        popNativeReg(1, true);
...
void X64Asm::bmi2ShiftLeftReg(U8 dstReg, U8 srcReg, bool isSrcRex, U8 amountReg) {
    // shlx tmpReg,regEmulatedAddress,12 
    U8 index = 7 - amountReg;
    write8(0xc4); // vex
    write8(isSrcRex ? 0x42 : 0x62);
    write8(0x81 | (index << 3));
...
void X64Asm::writeToRegFromMem(U8 dst, bool isDstRex, U8 reg2, bool isReg2Rex, S8 reg3, bool isReg3Rex, U8 reg3Shift, S32 displacement, U8 bytes, bool translateToHost, bool skipAlignmentCheck, bool releaseReg3) {
    if (translateToHost) {
        if (reg3 >= 0 || displacement) {
            ...
            addWithLea(emulatedAddressReg, true, reg2, isReg2Rex, reg3, isReg3Rex, reg3Shift, displacement, 4);
            if (releaseReg3) {
                releaseTmpReg(reg3);
            }
            U8 hostMemReg = getTmpReg();
            checkMemory(emulatedAddressReg, true, false, bytes, hostMemReg, skipAlignmentCheck, dst);
            writeToRegFromMem(dst, isDstRex, emulatedAddressReg, true, hostMemReg, true, 0, 0, bytes, false);
            releaseTmpReg(emulatedAddressReg);
            releaseTmpReg(hostMemReg);
        } else {
            U8 hostMemReg = getTmpReg();
            checkMemory(reg2, isReg2Rex, false, bytes, hostMemReg, skipAlignmentCheck);
...
void X64Asm::getRamPage(U8 memReg, U8 pageReg, bool isWrite) {
    // get mmu entry "mmu[page]"
    // mov memReg, HOST_MMU + memReg << 2
    if (isWrite) {
        writeToRegFromMem(memReg, true, HOST_MEM_READ, true, pageReg, true, 3, K_NUMBER_OF_PAGES * 8, 8, false);
    } else {
        writeToRegFromMem(memReg, true, HOST_MEM_READ, true, pageReg, true, 3, 0, 8, false);
    }
}

#ifdef BOXEDWINE_4K_PAGE_SIZE
void X64Asm::checkMemory4k(U8 emulatedAddressReg, bool isRex, bool isWrite, U32 width, U8 memReg, bool skipAlignmentCheck, bool needFlags) {
    ...
    if (needFlags) {
        shiftRightNoFlags(emulatedAddressReg, isRex, pageReg, K_PAGE_SHIFT, memReg);
    } else {
        writeToRegFromReg(pageReg, true, emulatedAddressReg, isRex, 4);
        shiftRightReg(pageReg, true, K_PAGE_SHIFT);
    }

    writeToRegFromMem(memReg, true, HOST_MEM_READ, true, pageReg, true, 3, K_NUMBER_OF_PAGES * (isWrite ? 24 : 16), 8, false);
    // test that memReg isn't null by causing an exception
    writeToRegFromMem(memReg, true, memReg, true, -1, false, 0, 0, 4, false);
...
void X64Asm::checkMemory(U8 emulatedAddressReg, bool isRex, bool isWrite, U32 width, U8 memReg, bool skipAlignmentCheck, U8 tmpReg) {
    ...
    if (!flagsWrittenToInstructionStoredFlags) {
        needFlags = needsToPreverveFlags();
    }
#ifdef BOXEDWINE_4K_PAGE_SIZE
    // fpu and mmx have some stability issues when used in platformThread.cpp / seh_filter
    if (cpu->use4kMemCheck && currentOp->exceptionCount < 0xff) {
        checkMemory4k(emulatedAddressReg, isRex, isWrite, width, memReg, skipAlignmentCheck, needFlags);
        return;
    }
#endif    
    ...
    if (width > 1 && !skipAlignmentCheck) {
        ...
        writeToRegFromReg(offsetReg, true, emulatedAddressReg, isRex, 4);
        andReg(offsetReg, true, 0xFFF);
    }

    // get page
    writeToRegFromReg(memReg, true, emulatedAddressReg, isRex, 4);
    shiftRightReg(memReg, true, K_PAGE_SHIFT);

    getRamPage(memReg, memReg, isWrite);
    ...
    if (width > 1 && !skipAlignmentCheck) {
        ...
        if (width == 2 || width == 4 || width == 8 || width == 16) {
            ...
            write8(0xC0 | HOST_CPU | (offsetReg << 3));
            if (width == 2) {
            ...
            write8(0x49);
            write8(0x81);
            write8(0xf8 | offsetReg);
            write32(0x1000 - width);

            zeroReg(offsetReg, true, true);
            ...
            write8(0x0f);
            write8(0x47);
            write8(0xC0 | (testReg << 3) | offsetReg);
        }
        if (offsetReg != tmpReg) {
    ...
    doIf(testReg, true, 0, [=, this] {
        ...
        }, nullptr, false, true, testReg != memReg);
...
void X64Asm::translateRM(U8 rm, bool checkG, bool checkE, bool isG8bit, bool isE8bit, U8 immWidth, bool calculateHostAddress) {
    if (rm < 0xC0) {
        ...
        if (!width) {
            width = 4; // i've only seen this of 0x11f, an sse hint that the decoder converts to a nop.  MDK with cnc ddraw triggers it.
        }
        if (this->ds == SEG_ZERO) {
            calculateMemory(G(rm), false, rm);
            skipWriteOp = true;
        } else if (calculateHostAddress && width) {
            U8 emulatedAddressReg = getTmpReg();

            calculateMemory(emulatedAddressReg, true, rm);

            U8 memReg = getTmpReg();
            checkMemory(emulatedAddressReg, true, isWrite, width, memReg, false);

            // memReg and emulatedAddressReg will be auto-release after this op
            writeHostPlusTmp((rm & (7 << 3)) | 4, checkG, isG8bit, isE8bit, emulatedAddressReg, calculateHostAddress, memReg);
        } else {
            int ii = 0;
        }
    } else {
...
void X64Asm::writeToRegFromE(U8 reg, bool isRegRex, U8 rm, U8 bytes) {
    if (rm < 0xC0) {
        ...
        U8 memReg = getTmpReg();
        checkMemory(reg, isRegRex, false, bytes, memReg, false);
        writeToRegFromMem(reg, isRegRex, reg, isRegRex, memReg, true, 0, 0, bytes, false);
...
void X64Asm::xlat() {
    ...
    // tmp = tmp + DS
    U8 memReg = getTmpReg();
    addWithLea(tmpReg, true, tmpReg, true, getRegForSeg(this->ds, memReg), true, 0, 0, 4);

    checkMemory(tmpReg, true, false, 1, memReg, false);
    // [HOST_MEM + HOST_TMP]
    writeHostPlusTmp(4, false, true, true, tmpReg, true, memReg);
    releaseTmpReg(memReg);
...
void X64Asm::popw(U8 rm) {
    if (rm < 0xC0) {
        U32 tmpReg = getTmpReg();
        if (currentOp->rm == 4 || currentOp->sibIndex == 4) {
            popReg(tmpReg, true, 2, true); // we update SP now since this op will reference it when calculating the address the popped result will be written to
            postCalculateMemory = [this]() {
                U8 tmpReg2 = getTmpReg();
                adjustStack(tmpReg2, -2); // rollback SP change before we do the write
                releaseTmpReg(tmpReg2);
                };
            writeToEFromReg(rm, tmpReg, true, 2);
            postCalculateMemory = nullptr;
        } else {
...
void X64Asm::popd(U8 rm) {
    if (rm < 0xC0) {
        U32 tmpReg = getTmpReg();
        if (currentOp->rm == 4 || currentOp->sibIndex == 4) {
            popReg(tmpReg, true, 4, true); // we update ESP now since this op will reference it when calculating the address the popped result will be written to
            postCalculateMemory = [this]() {
                U8 tmpReg2 = getTmpReg();
                adjustStack(tmpReg2, -4); // rollback ESP change before we do the write
                releaseTmpReg(tmpReg2);
                };
...
void X64Asm::writeToMemFromReg(U8 src, bool isSrcRex, U8 reg2, bool isReg2Rex, S8 reg3, bool isReg3Rex, U8 reg3Shift, S32 displacement, U8 bytes, bool translateToHost, bool skipAlignmentCheck, bool releaseReg3) {
    if (translateToHost) {
        if (reg3 >= 0 || displacement) {
            ...
            U8 memReg = getTmpReg();
            checkMemory(emulatedAddressReg, true, true, bytes, memReg, skipAlignmentCheck);
            writeToMemFromReg(src, isSrcRex, emulatedAddressReg, true, memReg, true, 0, 0, bytes, false);
            releaseTmpReg(emulatedAddressReg);
            releaseTmpReg(memReg);
        } else {
            U8 hostMemReg = getTmpReg();
            checkMemory(reg2, isReg2Rex, true, bytes, hostMemReg, skipAlignmentCheck);
            writeToMemFromReg(src, isSrcRex, reg2, isReg2Rex, hostMemReg, true, 0, 0, bytes, false);
            releaseTmpReg(hostMemReg);
        }
    } else {
        if (disp > 0x7fffffff) {
            kpanic("X64Asm::writeToMemFromReg untranslated memory access will treat disp as signed, which is not correct");
        }
...
void X64Asm::writeToMemFromValue(U64 value, U8 reg2, bool isReg2Rex, S8 reg3, bool isReg3Rex, U8 reg3Shift, S32 displacement, U8 bytes, bool translateToHost, bool canUseReg2AsTmp, bool skipAlignmentCheck) {
    if (translateToHost) {
        if (canUseReg2AsTmp) {
            ...
            checkMemory(reg2, isReg2Rex, true, bytes, hostMemReg, skipAlignmentCheck);
            writeToMemFromValue(value, reg2, true, hostMemReg, true, 0, 0, bytes, false);
            releaseTmpReg(hostMemReg);
        } else {
            U8 emulatedAddressReg = getTmpReg();
            U8 hostMemReg = getTmpReg();
            addWithLea(emulatedAddressReg, true, reg2, isReg2Rex, reg3, isReg3Rex, reg3Shift, displacement, 4);
            checkMemory(emulatedAddressReg, true, true, bytes, hostMemReg, skipAlignmentCheck);
            writeToMemFromValue(value, emulatedAddressReg, true, hostMemReg, true, 0, 0, bytes, false);
            releaseTmpReg(emulatedAddressReg);
            releaseTmpReg(hostMemReg);
        }
    } else {
...
void X64Asm::releaseTmpReg(U8 tmpReg) {
    ...
    } else if (tmpReg == HOST_TMP5) {
        if (!this->tmp5InUse) {
            kpanic("x64: tried to release HOST_TMP5 but it was not in use");
        }
        this->tmp5InUse = false;
    } else {
...
bool X64Asm::isTmpReg(U8 tmpReg) {
    return tmpReg == HOST_TMP || tmpReg == HOST_TMP2 || tmpReg == HOST_TMP3 || tmpReg == HOST_TMP4 || tmpReg == HOST_TMP5;
}
...
void X64Asm::createCodeForSyncFromHost() {
...
#ifdef BOXEDWINE_USE_SSE_FOR_FPU
    // store SSE to CPU
    zeroReg(tmpReg, true, false); // writeToRegFromMem with 1 byte will not zero extend (maybe in future all doIf to look at single byte)
    writeToRegFromMem(tmpReg, true, HOST_CPU, true, -1, false, 0, CPU_OFFSET_FPU_IS_MMX, 1, false);    
    for (int i = 0; i < 8; i++) {
    ...
    doIf(tmpReg, true, 0, [this] {
        U8 sizeofReg = cpu->fpu.sizeofRegInRegsArray();
        for (int i = 0; i < 8; i++) {
            // movq
            write8(0x41);
            write8(0x0f);
            write8(0x7f);
            write8(0x80 | HOST_CPU | (i << 3));
            write32((U32)(offsetof(CPU, fpu.regs[0])) + sizeofReg * i);
        }
...
void X64Asm::createCodeForSyncToHost() {
    ...
    doIf(tmpReg, true, 0, [this] {
        for (int i = 0; i < 8; i++) {
            ...
            write8(0x6f);
            write8(0x80 | HOST_CPU | (i << 3));
            write32((U32)(offsetof(CPU, fpu.regs[0].signif) + i * cpu->fpu.sizeofRegInRegsArray()));
        }
    }, nullptr, false, true, false, 0x74, 0x75); 
#else
    write8(0x41);
...
void X64Asm::syncRegsToHost(S8 excludeReg) {
    ...
    write32(CPU_OFFSET_FPU_STATE);
#endif
    writeToRegFromMem(tmpReg, true, HOST_CPU, true, -1, false, 0, CPU_OFFSET_HOST_FLAGS, 4, false);
    popFlagsFromReg(tmpReg, true, true);
    releaseTmpReg(tmpReg);
}
...
void X64Asm::testRegReg(U8 reg, bool isRegRex, U8 reg2, bool isReg2Rex, bool is64bit) {
    if (isRegRex || is64bit || isReg2Rex) {
        write8(REX_BASE | (isReg2Rex ? REX_MOD_REG : 0) | (isRegRex ? REX_MOD_RM : 0) | (is64bit ? REX_64 : 0));
    }
    write8(0x85);    
    write8(0xc0 | reg | (reg2 << 3));
}

void X64Asm::testReg(U8 reg, bool isRegRex, U64 value, bool is64bit) {
    if (value > 0xffffffffl && is64bit) {
        U8 tmp = getTmpReg();
        writeToRegFromValue(tmp, true, value, 8);
        testRegReg(reg, isRegRex, tmp, true, is64bit);
        releaseTmpReg(tmp);
    } else {
        if (isRegRex || is64bit) {
            U8 rex = REX_BASE;
...
void X64Asm::xorRegReg(U8 reg, bool isRegRex, U8 reg2, bool isReg2Rex, bool is64bit) {    
    if (isRegRex || is64bit || isReg2Rex) {
        write8(REX_BASE | (isRegRex ? REX_MOD_RM : 0) | (isReg2Rex ? REX_MOD_REG : 0) | (is64bit ? REX_64 : 0));
    }
    write8(0x31);
    write8(0xc0 | reg | (reg2 << 3));
}

void X64Asm::xorReg(U8 reg, bool isRegRex, U64 value, bool is64bit) {
    if (value > 0xffffffffl && is64bit) {
        U8 tmp = getTmpReg();
        writeToRegFromValue(tmp, true, value, 8);
        xorRegReg(reg, isRegRex, tmp, true, is64bit);
        releaseTmpReg(tmp);
    } else {
        if (isRegRex || is64bit) {
            write8(REX_BASE | (isRegRex ? REX_MOD_RM : 0) | (is64bit ? REX_64 : 0));
...
void X64Asm::clz(U8 dst, bool isDstRex, U8 src, bool isSrcRex, bool is64bit) {
    if (0) {
        // LZCNT
        write8(0xf3);
        if (isSrcRex || is64bit || isDstRex) {
            write8(REX_BASE | (isDstRex ? REX_MOD_REG : 0) | (isSrcRex ? REX_MOD_RM : 0) | (is64bit ? REX_64 : 0));
        }
        write8(0x45);
        write8(0x0f);
        write8(0xbd);
        write8(0xc0 | (dst << 3) | src);
    } else {
        testRegReg(src, isSrcRex, src, isSrcRex, is64bit);
        doIf(src, isSrcRex, 0, [this, src, isSrcRex] {
            zeroReg(src, isSrcRex, false);
        }, [this, dst, isDstRex, is64bit, src, isSrcRex]() {
            //bsr     dst, src
            if (isSrcRex || is64bit || isDstRex) {
                write8(REX_BASE | (isDstRex ? REX_MOD_REG : 0) | (isSrcRex ? REX_MOD_RM : 0) | (is64bit ? REX_64 : 0));
            }
            ...
            xorReg(dst, isDstRex, (is64bit?63:31), is64bit);
        }, false, false);
    }
}

void X64Asm::neg(U8 reg, bool isRegRex, bool is64bit) {
    if (isRegRex || is64bit) {
        write8(REX_BASE | (isRegRex ? REX_MOD_RM : 0) | (is64bit ? REX_64 : 0));
    }
    write8(0xf7);
...
void X64Asm::abs(U8 reg, bool isRegRex, bool is64bit) {
    U8 tmpReg = getTmpReg();
    writeToRegFromReg(tmpReg, true, reg, isRegRex, is64bit ? 8 : 4);
    neg(reg, isRegRex, is64bit);

    // cmovl reg, tmpReg
    write8(REX_BASE | REX_MOD_REG | (isRegRex ? REX_MOD_RM : 0) | (is64bit ? REX_64 : 0));
    write8(0x0f);
    write8(0x4c);
    write8(0xC0 | (reg << 3) | tmpReg);
}

void X64Asm::cmpReg(U8 reg, bool isRegRex, U64 value, bool is64bit) {
    if (value > 0xffffffffl && is64bit) {
        U8 tmp = getTmpReg();
        writeToRegFromValue(tmp, true, value, 8);

        U8 rex = REX_BASE | REX_64 | REX_MOD_REG;
        if (isRegRex) {
            rex += REX_MOD_RM;
        }
        write8(rex);
        write8(0x39);
        write8(0xc0 | reg | (tmp << 3));
        releaseTmpReg(tmp);
    } else {
        if (isRegRex || is64bit) {
            U8 rex = REX_BASE;
...
void X64Asm::shiftRightReg(U8 reg, bool isRegRex, U8 shiftAmount, bool arith, bool is64bit) {
    if (isRegRex || is64bit) {
        U8 rex = REX_BASE;
        if (isRegRex) {
            rex += REX_MOD_RM;
...
void X64Asm::jmpTmp4(bool mightNeedCS) {     
    // don't use HOST_TMP4 (reg = 7), jmpAndTranslateIfNecessary looks for eip there

    bool savedFlags = false;

    // so that exception handler will know where to look for next op if any null pointer exceptions happen below
    this->writeToMemFromReg(7, true, HOST_CPU, true, -1, false, 0, CPU_OFFSET_EIP, 4, false);

    // HOST_TMP5 is tmp
    // HOST_TMP4 will hold eip
    // HOST_TMP3 is tmp
    ...
    if (x64CPU::hasBMI2) {
        if (!this->cpu->thread->process->hasSetSeg[CS] && !mightNeedCS) {
            // mov HOST_TMP2, reg
            writeToRegFromReg(HOST_TMP2, true, HOST_TMP4, true, false);
            ...
            addWithLea(HOST_TMP2, true, HOST_TMP4, true, HOST_TMP2, true, 0, 0, 4);
        ...
        if (!this->cpu->thread->process->hasSetSeg[CS] && !mightNeedCS) {
            writeToRegFromReg(HOST_TMP, true, HOST_TMP4, true, 4);
            writeToRegFromReg(HOST_TMP2, true, HOST_TMP4, true, 4);
        } else {
            getRegForSeg(CS, HOST_TMP2);
            addWithLea(HOST_TMP2, true, HOST_TMP4, true, HOST_TMP2, true, 0, 0, 4);
        ...
        // :TODO: if HOST_TMP3 is used here instead of HOST_TMP4, it will crash.  Why?
        pushFlagsToReg(HOST_TMP4, true, true);
    ...
    writeToRegFromReg(HOST_TMP5, true, HOST_TMP2, true, 4);

    // HOST_TMP5 will hold 1st index
    // HOST_TMP2 will hold 2nd index
    if (x64CPU::hasBMI2) {
        shiftRightNoFlags(HOST_TMP5, true, HOST_TMP5, 10, HOST_TMP4);
        andRegNoFlags(HOST_TMP2, HOST_TMP2, 0x3ff, HOST_TMP4);
    } else {
        shiftRightReg(HOST_TMP5, true, 10);
        andReg(HOST_TMP2, true, 0x3ff);
        popFlagsFromReg(HOST_TMP4, true, true); // no exceptions can happen between pushFlagsToReg and here
    }
    ...
    // HOST_TMP3 = HOST_TMP3[HOST_TMP5 * 8]
    writeToRegFromMem(HOST_TMP3, true, HOST_TMP3, true, HOST_TMP5, true, 3, 0, 8, false);    
    ...
    // HOST_TMP3 now contains DecodedOp

    writeToRegFromMem(HOST_TMP3, true, HOST_TMP3, true, -1, false, 0, offsetof(DecodedOp, pfnJitCode), 8, false);

    // generate exception if null pointer here instead of in jump
    writeToRegFromMem(HOST_TMP5, true, HOST_TMP3, true, -1, false, 0, 0, 8, false);

    // jmp HOST_TMP
    jmpNativeReg(HOST_TMP3, true);
}

void X64Asm::jmpAddress(U32 eip) {
    ...
    writeToRegFromValue(tmpReg, true, (U64)pDestOp, 8);

    // read DecodedOp into tmpReg
    writeToRegFromMem(tmpReg, true, tmpReg, true, -1, false, 3, 0, 8, false);
    
    // get translated code address from DecodedOp in tmpReg
    writeToRegFromMem(tmpReg, true, tmpReg, true, -1, false, 0, offsetof(DecodedOp, pfnJitCode), 8, false);

    U8 tmpReg2 = getTmpReg();
    // generate exception if null pointer here instead of in jump
    writeToRegFromMem(tmpReg2, true, tmpReg, true, -1, false, 0, 0, 8, false);

    // jmp tmpReg
    jmpNativeReg(tmpReg, true);
    releaseTmpReg(tmpReg);
}
...
void X64Asm::clearDirectionFlag() {
    ...
    // and dword ptr[HOST_CPU + CPU_OFFSET_FLAGS], ~DF
    write8(0x41);
    write8(0x81);
    write8(0xa0 | HOST_CPU);
    write32(CPU_OFFSET_FLAGS);
    write32(~DF);
    if (needFlags) {
        popFlagsFromReg(flagsReg, true, true);
    }
...
void X64Asm::setDirectionFlag() {
    bool needFlags = needsToPreverveFlags();
    U8 flagsReg = getTmpReg();
    if (needFlags) {
        pushFlagsToReg(flagsReg, true, true);
    }
    // or byte ptr[HOST_CPU + CPU_OFFSET_FLAGS + 1], 0x4
    write8(0x41);
    write8(0x83);
    write8(0x88 | HOST_CPU);
    write32(CPU_OFFSET_FLAGS + 1);
    write8(4);
    if (needFlags) {
        popFlagsFromReg(flagsReg, true, true);
    }
...
void X64Asm::string(U32 width, bool hasSrc, bool hasDst) {
    ...
    if (needFlags) {
        ...
        releaseTmpReg(flagsReg);
        flagsWrittenToInstructionStoredFlags = true;
    }
    if (repeat) {
        // test (e)cx, (e)cx
        if (ea16) {
    ...
    {
        U8 tmp = getTmpReg();
        writeToRegFromMem(tmp, true, HOST_CPU, true, -1, false, 0, CPU_OFFSET_FLAGS, 4, false);
    ...
    if (hasSrc && hasDst) {
        ...
        if (this->cpu->thread->process->hasSetSeg[ES]) {
            if (tmpDst == 0xff) {
                ...
                dstReg = tmpDst;
                dstRegIsRex = true;
                addWithLea(dstReg, true, 7, false, getRegForSeg(ES, dstReg), true, 0, 0, 4);
            } else {
                U8 segReg = getTmpReg();
                addWithLea(dstReg, true, dstReg, true, getRegForSeg(ES, segReg), true, 0, 0, 4);
...
static void x64_jmpAndTranslateIfNecessary() {
    ...
    try {
        ...
        DecodedOp* op = cpu->memory->getDecodedOp(address);
        DecodedOp* opPlus1 = cpu->memory->getDecodedOp(address + 1);
        if (!op && opPlus1) {
            op = cpu->getNextOp();
            if (op->lock) {
                kpanic("TODO"); // maybe replace opPlus1 with op
            }
        }
    } catch (...) {
        int ii = 0;
...
void X64Asm::translateInstruction() {
    ...
    if (cpu->memory->isAddressDynamic(ip, currentOp->len)) {
        // mem->codeCache.markAddressDynamic(ip, currentOp->len);
    ...
    while (1) {
        ...
        if (!x64Decoder[this->inst](this)) {
            ...
            if (mmxWrite && !mmxRead) {
                ...
                writeToMemFromValue(0, HOST_CPU, true, -1, false, 0, CPU_OFFSET_FPU_TOP, 1, false);
                writeToMemFromValue(0, HOST_CPU, true, -1, false, 0, CPU_OFFSET_FPU_SW, 1, false);
            }
            break;
        }
    }    
