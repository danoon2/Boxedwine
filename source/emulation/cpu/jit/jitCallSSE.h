...
#undef SSE_0
#define SSE_0(name) void Jit::dynamic_##name(DecodedOp* op) {call(common_##name);incrementEip(op->len);}
#undef SSE_RR
#define SSE_RR(name) void Jit::dynamic_##name(DecodedOp* op) {call_II(common_##name, op->reg, op->rm);incrementEip(op->len);}
