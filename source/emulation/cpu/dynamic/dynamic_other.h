...
void OPCALL dynamic_sahf(CPU* cpu, DecodedOp* op) {
    ...
    callHostFunction(NULL, common_setFlags, false, false, false, 3, 0, DYN_PARAM_CPU, offsetof(CPU, reg[0].h8), DYN_PARAM_CPU_ADDRESS_8, FMASK_ALL & 0xFF, DYN_PARAM_CONST_32);
    INCREMENT_EIP(op->len);
}
...
void OPCALL dynamic_retn16Iw(CPU* cpu, DecodedOp* op) {
    ...
    movToCpuFromReg(offsetof(CPU, eip.u32), DYN_CALL_RESULT, DYN_32bit); blockDone();
}
...
void OPCALL dynamic_callR16(CPU* cpu, DecodedOp* op) {
    ...
    movToRegFromReg(DYN_SRC, DYN_32bit, DYN_SRC, DYN_16bit); movToCpuFromReg(offsetof(CPU, eip.u32), DYN_SRC, DYN_32bit); blockDone();
}
void OPCALL dynamic_callR32(CPU* cpu, DecodedOp* op) {
    movToRegFromCpu(DYN_SRC, offsetof(CPU, eip.u32), DYN_32bit);
    instRegImm('+', DYN_SRC, DYN_32bit, op->len);
    callHostFunction(NULL, common_push32, false, false, false, 2, 0, DYN_PARAM_CPU, DYN_SRC, DYN_PARAM_REG_32);
    movToCpuFromCpu(offsetof(CPU, eip.u32), offsetof(CPU, reg[op->reg].u32), DYN_32bit, DYN_SRC); blockDone();
}
void OPCALL dynamic_callE16(CPU* cpu, DecodedOp* op) {
    ...
    movToCpuFromReg(offsetof(CPU, eip.u32), DYN_READ_RESULT, DYN_32bit); blockDone();
}
...
void OPCALL dynamic_jmpR16(CPU* cpu, DecodedOp* op) {
    ...
    movToCpuFromReg(offsetof(CPU, eip.u32), DYN_SRC, DYN_32bit); blockDone();
}
