...
#define SSE_RR_SETS_FLAGS(name) void dynamic_##name(DynamicData* data, DecodedOp* op) {data->currentLazyFlags=FLAGS_NONE;callHostFunction((void*)common_##name, false, 3, 0, DYN_PARAM_CPU, false, op->reg, DYN_PARAM_CONST_32, false, op->rm, DYN_PARAM_CONST_32, false);INCREMENT_EIP(data, op);}
#undef SSE_RE
...
#define SSE_RE_SETS_FLAGS(name) void dynamic_##name(DynamicData* data, DecodedOp* op) {data->currentLazyFlags=FLAGS_NONE;calculateEaa(op, DYN_ADDRESS);callHostFunction((void*)common_##name, false, 3, 0, DYN_PARAM_CPU, false, op->reg, DYN_PARAM_CONST_32, false, DYN_ADDRESS, DYN_PARAM_REG_32, true);INCREMENT_EIP(data, op);}
#undef SSE_RR_I8
