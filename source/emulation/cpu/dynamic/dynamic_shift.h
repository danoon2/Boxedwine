...
void DynamicData::dynamic_shl8_reg_op(DecodedOp* op) {
    if (!op->needsToSetFlags(cpu)) {
        instCPUImm('<', op->reg, DYN_8bit, op->imm, DYN_DEST);
        incrementEip(op->len);
        return;
    }
...
void DynamicData::dynamic_shl8cl_reg_op(DecodedOp* op) {
    if (!op->needsToSetFlags(cpu)) {
        ...
        instCPUReg('<', op->reg, reg, DYN_8bit, true, DYN_DEST);
        incrementEip(op->len);
        return;
    }
    loadReg(1, DYN_SRC, DYN_8bit, true);
...
void DynamicData::dynamic_shl8cl_mem_op(DecodedOp* op) {
    if (!op->needsToSetFlags(cpu)) {
        calculateEaa(op, DYN_ADDRESS);
        DynReg reg = loadReg(1, DYN_SRC, DYN_8bit);
        instMemReg('<', DYN_ADDRESS, reg, DYN_8bit, true, true, DYN_DEST);
        incrementEip(op->len);
        return;
    }
    loadReg(1, DYN_SRC, DYN_8bit, true);
...
void DynamicData::dynamic_shl16_reg_op(DecodedOp* op) {
    if (!op->needsToSetFlags(cpu)) {
        instCPUImm('<', op->reg, DYN_16bit, op->imm, DYN_DEST);
        incrementEip(op->len);
        return;
    }
...
void DynamicData::dynamic_shl16cl_reg_op(DecodedOp* op) {
    if (!op->needsToSetFlags(cpu)) {
        DynReg reg = loadReg(1, DYN_SRC, DYN_8bit);
        instCPUReg('<', op->reg, reg, DYN_16bit, true, DYN_DEST);
        incrementEip(op->len);
        return;
    }
    loadReg(1, DYN_SRC, DYN_8bit, true);
...
void DynamicData::dynamic_shl16cl_mem_op(DecodedOp* op) {
    if (!op->needsToSetFlags(cpu)) {
        calculateEaa(op, DYN_ADDRESS);
        DynReg reg = loadReg(1, DYN_SRC, DYN_8bit);
        instMemReg('<', DYN_ADDRESS, reg, DYN_16bit, true, true, DYN_DEST);
        incrementEip(op->len);
        return;
    }
    loadReg(1, DYN_SRC, DYN_8bit, true);
...
void DynamicData::dynamic_shl32_reg_op(DecodedOp* op) {
    if (!op->needsToSetFlags(cpu)) {
        instCPUImm('<', op->reg, DYN_32bit, op->imm, DYN_DEST);
        incrementEip(op->len);
        return;
    }
...
void DynamicData::dynamic_shl32cl_reg_op(DecodedOp* op) {
    if (!op->needsToSetFlags(cpu)) {
        DynReg reg = loadReg(1, DYN_SRC, DYN_8bit);
        instCPUReg('<', op->reg, reg, DYN_32bit, true, DYN_DEST);
        incrementEip(op->len);
        return;
    }
    loadReg(1, DYN_SRC, DYN_8bit, true);
...
void DynamicData::dynamic_shl32cl_mem_op(DecodedOp* op) {
    if (!op->needsToSetFlags(cpu)) {
        calculateEaa(op, DYN_ADDRESS);
        DynReg reg = loadReg(1, DYN_SRC, DYN_8bit);
        instMemReg('<', DYN_ADDRESS, reg, DYN_32bit, true, true, DYN_DEST);
        incrementEip(op->len);
        return;
    }
    loadReg(1, DYN_SRC, DYN_8bit, true);
