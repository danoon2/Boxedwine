void dynamic_inc8_reg(DynamicData* data, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        movToCpuLazyFlags(CPU_OFFSET_OF(lazyFlags), DYN_LAZY_FLAG(FLAGS_INC8));
...
void dynamic_inc16_reg(DynamicData* data, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        instCPUImm('+', CPU_OFFSET_OF_REG16(op->reg), DYN_16bit, 1);
    } else {
        dynamic_getCF(data);
        movToCpuFromReg(CPU_OFFSET_OF(oldCF), DYN_CALL_RESULT, DYN_32bit, true);
        movToCpuFromCpu(CPU_OFFSET_OF(dst.u16), CPU_OFFSET_OF_REG16(op->reg), DYN_16bit, DYN_DEST, false);
        ...
        movToCpuLazyFlags(CPU_OFFSET_OF(lazyFlags), DYN_LAZY_FLAG(FLAGS_INC16));
...
void dynamic_inc32_reg(DynamicData* data, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        instCPUImm('+', CPU_OFFSET_OF_REG32(op->reg), DYN_32bit, 1);
    } else {
        dynamic_getCF(data);
        movToCpuFromReg(CPU_OFFSET_OF(oldCF), DYN_CALL_RESULT, DYN_32bit, true);
        movToCpuFromCpu(CPU_OFFSET_OF(dst.u32), CPU_OFFSET_OF_REG32(op->reg), DYN_32bit, DYN_DEST, false);
        ...
        movToCpuLazyFlags(CPU_OFFSET_OF(lazyFlags), DYN_LAZY_FLAG(FLAGS_INC32));
...
void dynamic_dec8_reg(DynamicData* data, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        movToCpuFromReg(CPU_OFFSET_OF(result.u8), DYN_DEST, DYN_8bit, false);
        movToCpuFromReg(OFFSET_REG8(op->reg), DYN_DEST, DYN_8bit, true);
        movToCpuLazyFlags(CPU_OFFSET_OF(lazyFlags), DYN_LAZY_FLAG(FLAGS_DEC8));
...
void dynamic_dec16_reg(DynamicData* data, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        movToCpuFromReg(CPU_OFFSET_OF(result.u16), DYN_DEST, DYN_16bit, false);
        movToCpuFromReg(CPU_OFFSET_OF_REG16(op->reg), DYN_DEST, DYN_16bit, true);
        movToCpuLazyFlags(CPU_OFFSET_OF(lazyFlags), DYN_LAZY_FLAG(FLAGS_DEC16));
...
void dynamic_dec32_reg(DynamicData* data, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        movToCpuFromReg(CPU_OFFSET_OF(result.u32), DYN_DEST, DYN_32bit, false);
        movToCpuFromReg(CPU_OFFSET_OF_REG32(op->reg), DYN_DEST, DYN_32bit, true);
        movToCpuLazyFlags(CPU_OFFSET_OF(lazyFlags), DYN_LAZY_FLAG(FLAGS_DEC32));
