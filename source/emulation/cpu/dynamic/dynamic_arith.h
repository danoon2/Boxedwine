void OPCALL dynamic_addr8r8(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        instCPUReg('+', OFFSET_REG8(op->reg), DYN_SRC, DYN_8bit);
    } else {
...
void OPCALL dynamic_adde8r8(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        instMemReg('+', DYN_ADDRESS, DYN_SRC, DYN_8bit);
    } else {
...
void OPCALL dynamic_addr8e8(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        instCPUReg('+', OFFSET_REG8(op->reg), DYN_READ_RESULT, DYN_8bit);
    } else {
        calculateEaa(op, DYN_ADDRESS);
...
void OPCALL dynamic_add8_mem(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        calculateEaa(op, DYN_ADDRESS);
        instMemImm('+', DYN_ADDRESS, DYN_8bit, op->imm);
    } else {
        movToCpu(CPU_OFFSET_OF(src.u8), DYN_8bit, op->imm);
        calculateEaa(op, DYN_ADDRESS);
        movToCpuFromMem(CPU_OFFSET_OF(dst.u8), DYN_8bit, DYN_ADDRESS);
...
void OPCALL dynamic_addr16r16(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        instCPUReg('+', CPU_OFFSET_OF(reg[op->reg].u16), DYN_SRC, DYN_16bit);
    } else {
...
void OPCALL dynamic_adde16r16(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        instMemReg('+', DYN_ADDRESS, DYN_SRC, DYN_16bit);
    } else {
...
void OPCALL dynamic_addr16e16(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        instCPUReg('+', CPU_OFFSET_OF(reg[op->reg].u16), DYN_READ_RESULT, DYN_16bit);
    } else {
        calculateEaa(op, DYN_ADDRESS);
...
void OPCALL dynamic_add16_mem(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        calculateEaa(op, DYN_ADDRESS);
        instMemImm('+', DYN_ADDRESS, DYN_16bit, op->imm);
    } else {
        movToCpu(CPU_OFFSET_OF(src.u16), DYN_16bit, op->imm);
        calculateEaa(op, DYN_ADDRESS);
        movToCpuFromMem(CPU_OFFSET_OF(dst.u16), DYN_16bit, DYN_ADDRESS);
...
void OPCALL dynamic_addr32r32(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        instCPUReg('+', CPU_OFFSET_OF(reg[op->reg].u32), DYN_SRC, DYN_32bit);
    } else {
...
void OPCALL dynamic_adde32r32(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        instMemReg('+', DYN_ADDRESS, DYN_SRC, DYN_32bit);
    } else {
...
void OPCALL dynamic_addr32e32(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        ...
        instCPUReg('+', CPU_OFFSET_OF(reg[op->reg].u32), DYN_READ_RESULT, DYN_32bit);
    } else {
        calculateEaa(op, DYN_ADDRESS);
...
void OPCALL dynamic_add32_mem(CPU* cpu, DecodedOp* op) {
    if (!op->needsToSetFlags()) {
        calculateEaa(op, DYN_ADDRESS);
        instMemImm('+', DYN_ADDRESS, DYN_32bit, op->imm);
    } else {
        movToCpu(CPU_OFFSET_OF(src.u32), DYN_32bit, op->imm);
        calculateEaa(op, DYN_ADDRESS);
        movToCpuFromMem(CPU_OFFSET_OF(dst.u32), DYN_32bit, DYN_ADDRESS);
...
void OPCALL dynamic_adc8_reg(CPU* cpu, DecodedOp* op) {
    ...
    if (!op->needsToSetFlags()) {
        ...
        instCPUReg('+', OFFSET_REG8(op->reg), DYN_CALL_RESULT, DYN_8bit);
    } else {
        movToCpu(CPU_OFFSET_OF(src.u8), DYN_8bit, op->imm);
        movToCpuFromCpu(CPU_OFFSET_OF(dst.u8), OFFSET_REG8(op->reg), DYN_8bit, DYN_DEST);
        instRegImm('+', DYN_DEST, DYN_8bit, op->imm);
...
void OPCALL dynamic_adc8_mem(CPU* cpu, DecodedOp* op) {
    ...
    if (!op->needsToSetFlags()) {
        ...
        movToRegFromReg(DYN_CALL_RESULT, DYN_8bit, DYN_CALL_RESULT, DYN_32bit);
          instRegImm('+', DYN_CALL_RESULT, DYN_8bit, op->imm);
        instMemReg('+', DYN_ADDRESS, DYN_CALL_RESULT, DYN_8bit);
    } else {
        movToCpu(CPU_OFFSET_OF(src.u8), DYN_8bit, op->imm);
        calculateEaa(op, DYN_ADDRESS);
        movToCpuFromMem(CPU_OFFSET_OF(dst.u8), DYN_8bit, DYN_ADDRESS);
...
void OPCALL dynamic_adc16_reg(CPU* cpu, DecodedOp* op) {
    ...
    if (!op->needsToSetFlags()) {
        ...
        instCPUReg('+', CPU_OFFSET_OF(reg[op->reg].u16), DYN_CALL_RESULT, DYN_16bit);
    } else {
        movToCpu(CPU_OFFSET_OF(src.u16), DYN_16bit, op->imm);
        movToCpuFromCpu(CPU_OFFSET_OF(dst.u16), CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit, DYN_DEST);
        instRegImm('+', DYN_DEST, DYN_16bit, op->imm);
...
void OPCALL dynamic_adc16_mem(CPU* cpu, DecodedOp* op) {
    ...
    if (!op->needsToSetFlags()) {
        ...
        movToRegFromReg(DYN_CALL_RESULT, DYN_16bit, DYN_CALL_RESULT, DYN_32bit);
          instRegImm('+', DYN_CALL_RESULT, DYN_16bit, op->imm);
        instMemReg('+', DYN_ADDRESS, DYN_CALL_RESULT, DYN_16bit);
    } else {
        movToCpu(CPU_OFFSET_OF(src.u16), DYN_16bit, op->imm);
        calculateEaa(op, DYN_ADDRESS);
        movToCpuFromMem(CPU_OFFSET_OF(dst.u16), DYN_16bit, DYN_ADDRESS);
...
void OPCALL dynamic_adcr32r32(CPU* cpu, DecodedOp* op) {
    ...
    if (!op->needsToSetFlags()) {
        movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(reg[op->rm].u32), DYN_32bit);
        instRegReg('+', DYN_SRC, DYN_CALL_RESULT, DYN_32bit);
        instCPUReg('+', CPU_OFFSET_OF(reg[op->reg].u32), DYN_SRC, DYN_32bit);
    } else {
        movToCpuFromCpu(CPU_OFFSET_OF(src.u32), CPU_OFFSET_OF(reg[op->rm].u32), DYN_32bit, DYN_SRC);
        movToCpuFromCpu(CPU_OFFSET_OF(dst.u32), CPU_OFFSET_OF(reg[op->reg].u32), DYN_32bit, DYN_DEST);
...
void OPCALL dynamic_adc32_reg(CPU* cpu, DecodedOp* op) {
    ...
    if (!op->needsToSetFlags()) {
        ...
        instCPUReg('+', CPU_OFFSET_OF(reg[op->reg].u32), DYN_CALL_RESULT, DYN_32bit);
    } else {
        movToCpu(CPU_OFFSET_OF(src.u32), DYN_32bit, op->imm);
        movToCpuFromCpu(CPU_OFFSET_OF(dst.u32), CPU_OFFSET_OF(reg[op->reg].u32), DYN_32bit, DYN_DEST);
        instRegImm('+', DYN_DEST, DYN_32bit, op->imm);
...
void OPCALL dynamic_mulR16(CPU* cpu, DecodedOp* op) {
    callHostFunction(NULL, common_mul16, false, false, false, 2, 0, DYN_PARAM_CPU, CPU_OFFSET_OF(reg[op->reg].u16), DYN_PARAM_CPU_ADDRESS_16);
    INCREMENT_EIP(op->len);
}
...
void OPCALL dynamic_imulR16(CPU* cpu, DecodedOp* op) {
    callHostFunction(NULL, common_imul16, false, false, false, 2, 0, DYN_PARAM_CPU, CPU_OFFSET_OF(reg[op->reg].u16), DYN_PARAM_CPU_ADDRESS_16);
    INCREMENT_EIP(op->len);
}
...
void OPCALL dynamic_mulR32(CPU* cpu, DecodedOp* op) {
    callHostFunction(NULL, common_mul32, false, false, false, 2, 0, DYN_PARAM_CPU, CPU_OFFSET_OF(reg[op->reg].u32), DYN_PARAM_CPU_ADDRESS_32);
    INCREMENT_EIP(op->len);
}
...
void OPCALL dynamic_imulR32(CPU* cpu, DecodedOp* op) {
    callHostFunction(NULL, common_imul32, false, false, false, 2, 0, DYN_PARAM_CPU, CPU_OFFSET_OF(reg[op->reg].u32), DYN_PARAM_CPU_ADDRESS_32);
    INCREMENT_EIP(op->len);
}
...
void OPCALL dynamic_divR16(CPU* cpu, DecodedOp* op) {
    callHostFunction(blockDone, div16, true, false, true, 2, 0, DYN_PARAM_CPU, CPU_OFFSET_OF(reg[op->reg].u16), DYN_PARAM_CPU_ADDRESS_16);
    INCREMENT_EIP(op->len);
}
...
void OPCALL dynamic_idivR16(CPU* cpu, DecodedOp* op) {
    callHostFunction(blockDone, idiv16, true, false, true, 2, 0, DYN_PARAM_CPU, CPU_OFFSET_OF(reg[op->reg].u16), DYN_PARAM_CPU_ADDRESS_16);
    INCREMENT_EIP(op->len);
}
...
void OPCALL dynamic_divR32(CPU* cpu, DecodedOp* op) {
    callHostFunction(blockDone, div32, true, false, true, 2, 0, DYN_PARAM_CPU, CPU_OFFSET_OF(reg[op->reg].u32), DYN_PARAM_CPU_ADDRESS_32);
    INCREMENT_EIP(op->len);
}
...
void OPCALL dynamic_idivR32(CPU* cpu, DecodedOp* op) {
    callHostFunction(blockDone, idiv32, true, false, true, 2, 0, DYN_PARAM_CPU, CPU_OFFSET_OF(reg[op->reg].u32), DYN_PARAM_CPU_ADDRESS_32);
    INCREMENT_EIP(op->len);
}
