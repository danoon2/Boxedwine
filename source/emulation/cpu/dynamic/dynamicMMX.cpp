...
void DynamicCodeGenMMX::startMMX() {
    ...
    writeCPUValue(DYN_8bit, offsetof(CPU, fpu.isMMXInUse), 1);
    writeCPUValue(DYN_32bit, offsetof(CPU, fpu.isRegCached), 0);
    writeCPUValue(DYN_32bit, offsetof(CPU, fpu.isRegCached) + 4, 0);
    writeCPUValue(DYN_32bit, offsetof(CPU, fpu.tags[0]), 0);
    writeCPUValue(DYN_32bit, offsetof(CPU, fpu.tags[0]) + 4, 0);
}
...
void DynamicCodeGenMMX::dynamic_maskmovqEDIMmxMmx(DecodedOp* op) {
    ...
    write(DYN_64bit, std::move(address), nullptr, [op, this](RegPtr address, RegPtr offset) {
        loadCpuMMXReg(DynMMXReg(op->reg), op->reg);
        loadCpuMMXReg(DynMMXReg(op->rm), op->rm);
