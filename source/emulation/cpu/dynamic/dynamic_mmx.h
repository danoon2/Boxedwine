...
#undef MMX_RE
#define MMX_RE(name) void DynamicData::dynamic_##name(DecodedOp* op) {call_IR(common_##name, op->reg, DYN_32bit, calculateEaa(op));incrementEip(op->len);}
#undef MMX_RI
#define MMX_RI(name) void DynamicData::dynamic_##name(DecodedOp* op) {call_II(common_##name, op->reg, op->imm);incrementEip(op->len);}
