void dynamic_cmovO_16_reg(DynamicData* data, DecodedOp* op) {
    ...
    movToCpuFromCpu(CPU_OFFSET_OF_REG16(op->reg), CPU_OFFSET_OF_REG16(op->rm), DYN_16bit, DYN_SRC, true);
    endIf();
...
void dynamic_cmovO_16_mem(DynamicData* data, DecodedOp* op) {
    ...
    setConditionInReg(data, O, DYN_CALL_RESULT);
    startIf(DYN_CALL_RESULT, DYN_NOT_EQUALS_ZERO, true);
    movToCpuFromMem(CPU_OFFSET_OF_REG16(op->reg), DYN_16bit, DYN_ADDRESS, true, true);
    endIf();
    INCREMENT_EIP(data, op);
}
void dynamic_cmovO_32_reg(DynamicData* data, DecodedOp* op) {
    setConditionInReg(data, O, DYN_CALL_RESULT);
    startIf(DYN_CALL_RESULT, DYN_NOT_EQUALS_ZERO, true);
    movToCpuFromCpu(CPU_OFFSET_OF_REG32(op->reg), CPU_OFFSET_OF_REG32(op->rm), DYN_32bit, DYN_SRC, true);
    endIf();
    INCREMENT_EIP(data, op);
}
