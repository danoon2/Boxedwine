...
void dynamic_cmovO_16_reg(DynamicData* data, DecodedOp* op) {
    ...
    movToCpuFromCpu(data, CPU::offsetofReg16(op->reg), CPU::offsetofReg16(op->rm), DYN_16bit, DYN_SRC, true);
    EndIf(data);
...
void dynamic_cmovO_16_mem(DynamicData* data, DecodedOp* op) {
    ...
    setConditionInReg(data, O, DYN_CALL_RESULT);
    If(data, DYN_CALL_RESULT, true);
    movToCpuFromMem(data, CPU::offsetofReg16(op->reg), DYN_16bit, DYN_ADDRESS, true, true);
    EndIf(data);
    INCREMENT_EIP(data, op);
}
void dynamic_cmovO_32_reg(DynamicData* data, DecodedOp* op) {
    setConditionInReg(data, O, DYN_CALL_RESULT);
    If(data, DYN_CALL_RESULT, true);
    movToCpuFromCpu(data, CPU::offsetofReg32(op->reg), CPU::offsetofReg32(op->rm), DYN_32bit, DYN_SRC, true);
    EndIf(data);
    INCREMENT_EIP(data, op);
}
