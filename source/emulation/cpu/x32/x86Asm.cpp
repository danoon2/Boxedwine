...
void X86Asm::mem32(U8 inst, Reg32 dst, Reg32 rm, Reg32 sib, U8 shift, U32 disp) {
    ...
    if (shift == 0) {
        ...
        outb(0x40 | (sib.reg << 3) | rm.reg);
    } else {
        outb(0x80 | (sib.reg << 3) | rm.reg);
    }
    if (hasDisp) {
        if (small) {
            outb(disp);
...
void X86Asm::addMem(Reg16 dst, Reg32 rm, U32 disp) {
    outb(0x66);
    mem32(0x03, Reg32(dst.reg), rm, disp);
}

void X86Asm::addMem(Reg32 dst, Reg32 rm, U32 disp) {
    mem32(0x03, dst, rm, disp);
}

void X86Asm::addMem(Reg32 rm, U32 disp, U32 value) {
