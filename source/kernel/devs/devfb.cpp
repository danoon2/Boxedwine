...
U32 DevFB::map(KThread* thread, U32 address, U32 len, S32 prot, S32 flags, U64 off) {
    ...
    U8* nativeAddress = screenPixels;
    for (U32 i=0;i<pageCount;i++) {
        if (mem->memInfo[i+pageStart].type!=(U32)PageType::Invalid_Page && mem->memInfo[i + pageStart].type != (U32)PageType::RAM_Page) {
            kpanic("Something else got mapped into the framebuffer address");
        }
        mem->memInfo[pageStart + i].flags = PAGE_MAPPED | PAGE_READ | PAGE_WRITE;
        mem->memInfo[pageStart + i].type = (U32)PageType::RAM_Page;
        mem->memInfo[pageStart + i].ramPageIndex = ramPageAllocNative(nativeAddress);
        nativeAddress += K_PAGE_SIZE;
    }    
