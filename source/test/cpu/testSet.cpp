...
constexpr U32 REG_GUARD = 0x11112222;
constexpr U32 MEM_DST = 0x0200;
...
void emitSetReg(U8 opcode, int reg) {
    pushCode8(0x0f);
    pushCode8(opcode);
    pushCode8(0xc0 | reg);
}

void emitSetMem(U8 opcode, U32 address, bool big) {
    pushCode8(0x0f);
    pushCode8(opcode);
...
void applySetResult(U32* expectedRegs, int reg, bool taken) {
    applyRegValue(expectedRegs, reg, 8, taken ? 1 : 0);
}

void runRegCase(U8 opcode, bool big, int reg, U32 flags, bool assertFlags, bool trailingCmp, const char* name) {
    U32 expectedRegs[8];
    bool taken = conditionTaken(opcode, flags);

    newInstruction(flags);
    cpu->big = big;
    emitSetReg(opcode, reg);
    if (trailingCmp) {
        emitTrailingCmp();
    }
    initRegisters(expectedRegs);
    applySetResult(expectedRegs, reg, taken);
...
void runMemCase(U8 opcode, bool big, U32 flags, bool assertFlags, bool trailingCmp, const char* name) {
    U32 expectedRegs[8];
    bool taken = conditionTaken(opcode, flags);
    U32 expectedMem = (REG_GUARD & 0xffffff00) | (taken ? 1 : 0);

    newInstruction(flags);
    cpu->big = big;
    emitSetMem(opcode, MEM_DST, big);
    if (trailingCmp) {
        emitTrailingCmp();
    }
    initRegisters(expectedRegs);
    memory->writed(TEST_HEAP_ADDRESS + MEM_DST, REG_GUARD);

    runTestCPU();
    verifyRegisters(cpu, expectedRegs, name);
    if (memory->readd(TEST_HEAP_ADDRESS + MEM_DST) != expectedMem) {
        failed("%s memory destination", name);
    }
    if (assertFlags) {
        verifyFlags(flags, name);
    }
...
void runCmpRegCase(U8 opcode, bool big, int reg, const CmpCase& data, bool trailingCmp, const char* name) {
    U32 expectedRegs[8];
    int width = big ? 32 : 16;
...
void runCmpMemCase(U8 opcode, bool big, const CmpCase& data, bool trailingCmp, const char* name) {
    U32 expectedRegs[8];
    int width = big ? 32 : 16;
    U32 expectedFlags = flagsForCmp(data.lhs, data.rhs, width);
    bool taken = conditionTaken(opcode, expectedFlags);
...
void runSetOpcode(U8 opcode, bool big, const char* name) {
    ...
    for (size_t i = 0; i < caseCount(CMP_CASES); ++i) {
        ...
        runCmpMemCase(opcode, big, CMP_CASES[i], false, name);
        runCmpMemCase(opcode, big, CMP_CASES[i], true, name);
    }
}

void runSetWidth(bool big) {
    for (U8 opcode = 0x90; opcode <= 0x9f; ++opcode) {
        runSetOpcode(opcode, big, big ? "set e8 32-bit address" : "set e8 16-bit address");
    }
}
