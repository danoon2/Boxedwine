...
class BoxedApp {
public:
    BoxedApp() : bpp(32), fullScreen(FULLSCREEN_NOTSET), vsync(VSYNC_NOT_SET), dpiAware(false), showWindowImmediately(false), scale(0), scaleQuality(0), cpuAffinity(0), pollRate(-1), skipFramesFPS(0), container(NULL) {}
    BoxedApp(const std::string& name, const std::string& path, const std::string& cmd, BoxedContainer* container) : name(name), path(path), cmd(cmd), bpp(0), fullScreen(FULLSCREEN_NOTSET), vsync(VSYNC_NOT_SET), dpiAware(false), showWindowImmediately(false), scale(0), scaleQuality(0), cpuAffinity(0), pollRate(-1), skipFramesFPS(0), container(container) {}
