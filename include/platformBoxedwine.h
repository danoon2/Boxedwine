...
public:
    ...
    static void resetTestInstructionCacheClearCount();
    static U32 getTestInstructionCacheClearCount();
#endif
...
INLINE void safe_strcpy(char* dest, const char* src, size_t bufferSize) {
    ...
    if (len+1>bufferSize) {
        kpanic_fmt("safe_strcpy failed to copy %s, buffer is %zu bytes", src, bufferSize);
    }
    strcpy(dest, src);
}
...
INLINE void safe_strcat(char* dest, const char* src, size_t bufferSize) {
    ...
    if (len+1>bufferSize) {
        kpanic_fmt("safe_strcat failed to copy %s, buffer is %zu bytes", src, bufferSize);
    }
    strcat(dest, src);
}
