...
public class Other extends Base {
    ...
    public void generateAll(FileOutputStream fos, FileOutputStream fos_init, FileOutputStream fos32, FileOutputStream fosOps_h, FileOutputStream fosOps_cpp) throws IOException {
        ...
        inst(fos, fos_init, "daa", "Daa", "daa(cpu);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, daa, false, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "das", "Das", "das(cpu);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, das, false, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "aaa", "Aaa", "aaa(cpu);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, aaa, false, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "aas", "Aas", "aas(cpu);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, aas, false, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "aad", "Aad", "aad(cpu, op->imm);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, aad, false, false, false, 2, 0, DYN_PARAM_CPU, op->imm, DYN_PARAM_CONST_32);", true, false);
        inst(fos, fos_init, "aam", "Aam", "if (aam(cpu, op->imm)) {\r\n        NEXT();\r\n    } else {\r\n        NEXT_DONE();\r\n    }", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(blockDone, aam, true, false, true, 2, 0, DYN_PARAM_CPU, op->imm, DYN_PARAM_CONST_32);", true, false);
        ...
        inst(fos, fos_init, "cwd", "Cwd", "if (((S16)AX) < 0)\r\n        DX = 0xFFFF;\r\n    else\r\n        DX = 0;\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "if (((S16)AX) < 0) {\r\n        DX = 0xFFFF;\r\n    } else {\r\n        DX = 0;\r\n    }", "", "callHostFunction(NULL, common_cwd, false, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "cwq", "Cwq", "if (((S32)EAX) < 0)\r\n        EDX = 0xFFFFFFFF;\r\n    else\r\n        EDX = 0;\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "if (((S32)EAX) < 0) {\r\n        EDX = 0xFFFFFFFF;\r\n    } else {\r\n        EDX = 0;\r\n    }", "", "callHostFunction(NULL, common_cwq, false, false, false, 1, 0, DYN_PARAM_CPU);", true, false);

        inst(fos, fos_init, "callAp", "CallAp", "cpu->call(0, op->imm, op->disp, cpu->eip.u32+op->len);\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(eip.u32), DYN_32bit);\r\n    instRegImm('+', DYN_SRC, DYN_32bit, op->len);\r\n    callHostFunction(NULL, common_call, false, false, false, 5, 0, DYN_PARAM_CPU, 0, DYN_PARAM_CONST_32, op->imm, DYN_PARAM_CONST_32, op->disp, DYN_PARAM_CONST_32, DYN_SRC, DYN_PARAM_REG_32);\r\n    blockDone();", false, false);
        inst(fos, fos_init, "callFar", "CallFar", "cpu->call(1, op->imm, op->disp, cpu->eip.u32+op->len);\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(eip.u32), DYN_32bit);\r\n    instRegImm('+', DYN_SRC, DYN_32bit, op->len);\r\n    callHostFunction(NULL, common_call, false, false, false, 5, 0, DYN_PARAM_CPU, 1, DYN_PARAM_CONST_32, op->imm, DYN_PARAM_CONST_32, op->disp, DYN_PARAM_CONST_32, DYN_SRC, DYN_PARAM_REG_32);\r\n    blockDone();", false, false);
        inst(fos, fos_init, "jmpAp", "JmpAp", "cpu->jmp(0, op->imm, op->disp, cpu->eip.u32+op->len);\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(eip.u32), DYN_32bit);\r\n    instRegImm('+', DYN_SRC, DYN_32bit, op->len);\r\n    callHostFunction(NULL, common_jmp, false, false, false, 5, 0, DYN_PARAM_CPU, 0, DYN_PARAM_CONST_32, op->imm, DYN_PARAM_CONST_32, op->disp, DYN_PARAM_CONST_32, DYN_SRC, DYN_PARAM_REG_32);\r\n    blockDone();", false, false);
        inst(fos, fos_init, "jmpFar", "JmpFar", "cpu->jmp(1, op->imm, op->disp, cpu->eip.u32+op->len);\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(eip.u32), DYN_32bit);\r\n    instRegImm('+', DYN_SRC, DYN_32bit, op->len);\r\n     callHostFunction(NULL, common_jmp, false, false, false, 5, 0, DYN_PARAM_CPU, 1, DYN_PARAM_CONST_32, op->imm, DYN_PARAM_CONST_32, op->disp, DYN_PARAM_CONST_32, DYN_SRC, DYN_PARAM_REG_32);\r\n    blockDone();", false, false);
        inst(fos, fos_init, "retf16", "Retf16", "cpu->eip.u32+=op->len; cpu->ret(0, op->imm);\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(eip.u32), DYN_32bit);\r\n    instRegImm('+', DYN_SRC, DYN_32bit, op->len);\r\n    movToCpuFromReg(CPU_OFFSET_OF(eip.u32), DYN_SRC, DYN_32bit);\r\n    callHostFunction(NULL, common_ret, false, false, false, 3, 0, DYN_PARAM_CPU, 0, DYN_PARAM_CONST_32, op->imm, DYN_PARAM_CONST_32);\r\n    blockDone();", false, false);
        inst(fos, fos_init, "retf32", "Retf32", "cpu->eip.u32+=op->len; cpu->ret(1, op->imm);\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(eip.u32), DYN_32bit);\r\n    instRegImm('+', DYN_SRC, DYN_32bit, op->len);\r\n    movToCpuFromReg(CPU_OFFSET_OF(eip.u32), DYN_SRC, DYN_32bit);\r\n    callHostFunction(NULL, common_ret, false, false, false, 3, 0, DYN_PARAM_CPU, 1, DYN_PARAM_CONST_32, op->imm, DYN_PARAM_CONST_32);\r\n    blockDone();", false, false);
        inst(fos, fos_init, "iret", "Iret", "cpu->iret(0, cpu->eip.u32+op->len);\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(eip.u32), DYN_32bit);\r\n    instRegImm('+', DYN_SRC, DYN_32bit, op->len);\r\n    callHostFunction(NULL, common_iret, false, false, false, 3, 0, DYN_PARAM_CPU, 0, DYN_PARAM_CONST_32, DYN_SRC, DYN_PARAM_REG_32);\r\n    blockDone();", false, false);
        inst(fos, fos_init, "iret32", "Iret32", "cpu->iret(1, cpu->eip.u32+op->len);\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(eip.u32), DYN_32bit);\r\n    instRegImm('+', DYN_SRC, DYN_32bit, op->len);\r\n    callHostFunction(NULL, common_iret, false, false, false, 3, 0, DYN_PARAM_CPU, 1, DYN_PARAM_CONST_32, DYN_SRC, DYN_PARAM_REG_32);\r\n    blockDone();", false, false);

        inst(fos, fos_init, "sahf", "Sahf", "cpu->fillFlags(); cpu->setFlags(AH, FMASK_ALL & 0xFF);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, common_fillFlags, false, false, false, 1, 0, DYN_PARAM_CPU);\r\n    callHostFunction(NULL, common_setFlags, false, false, false, 3, 0, DYN_PARAM_CPU, CPU_OFFSET_OF(reg[0].h8), DYN_PARAM_CPU_ADDRESS_8, FMASK_ALL & 0xFF, DYN_PARAM_CONST_32);", true, false);
        inst(fos, fos_init, "lahf", "Lahf", "cpu->fillFlags(); AH = (cpu->flags & (SF|ZF|AF|PF|CF)) | 2;\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, common_fillFlags, false, false, false, 1, 0, DYN_PARAM_CPU);\r\n    movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(flags), DYN_32bit);\r\n    instRegImm('&', DYN_SRC, DYN_32bit, SF|ZF|AF|PF|CF);\r\n    instRegImm('|', DYN_SRC, DYN_32bit, 2);\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[0].h8), DYN_SRC, DYN_8bit);", true, false);
        inst(fos, fos_init, "salc", "Salc", "if (cpu->getCF()) AL = 0xFF; else AL = 0;\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "if (cpu->getCF()) AL = 0xFF; else AL = 0;", "", "callHostFunction(NULL, common_salc, false, false, false, 1, 0, DYN_PARAM_CPU);", true, false);

        inst(fos, fos_init, "retn16Iw", "Retn16Iw", "U16 eip = cpu->pop16();\r\n    SP = SP+op->imm;\r\n    cpu->eip.u32 = eip;\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, common_pop16, true, false, false, 1, 0, DYN_PARAM_CPU);\r\n    movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(reg[4].u16), DYN_16bit);\r\n    instRegImm('+', DYN_SRC, DYN_16bit, op->imm);\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[4].u16), DYN_SRC, DYN_16bit);\r\n    movToCpuFromReg(CPU_OFFSET_OF(eip.u32), DYN_CALL_RESULT, DYN_32bit); blockDone();", false, false);
        inst(fos, fos_init, "retn32Iw", "Retn32Iw", "U32 eip = cpu->pop32();\r\n    ESP = ESP+op->imm;\r\n    cpu->eip.u32 = eip;\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, common_pop32, true, false, false, 1, 0, DYN_PARAM_CPU);\r\n    movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(reg[4].u32), DYN_32bit);\r\n    instRegImm('+', DYN_SRC, DYN_32bit, op->imm);\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[4].u32), DYN_SRC, DYN_32bit);\r\n    movToCpuFromReg(CPU_OFFSET_OF(eip.u32), DYN_CALL_RESULT, DYN_32bit); blockDone();", false, false);
        ...
        inst(fos, fos_init, "int80", "Int80", "ksyscall(cpu, op->len);\r\n NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, ksyscall, false, false, false, 2, 0, DYN_PARAM_CPU, op->len, DYN_PARAM_CONST_32);\r\n    blockDone();", false, false);
        inst(fos, fos_init, "int98", "Int98", "U32 index = cpu->peek32(0);\r\n    if (index<wine_callbackSize && wine_callback[index]) {\r\n        wine_callback[index](cpu);\r\n    } else {\r\n        kpanic(\"Uknown int 98 call: %d\", index);\r\n    }\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "U32 index = cpu->peek32(0);\r\n    if (index<wine_callbackSize && wine_callback[index]) {\r\n        wine_callback[index](cpu);\r\n    } else {\r\n        kpanic(\"Uknown int 98 call: %d\", index);\r\n    }", "", "callHostFunction(NULL, common_int98, false, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "int99", "Int99", "U32 index = cpu->peek32(0);\r\n    if (index<int99CallbackSize && int99Callback[index]) {\r\n        int99Callback[index](cpu);\r\n    } else {\r\n        kpanic(\"Uknown int 99 call: %d\", index);\r\n    }\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "U32 index = cpu->peek32(0);\r\n    if (index<int99CallbackSize && int99Callback[index]) {\r\n        int99Callback[index](cpu);\r\n    } else {\r\n        kpanic(\"Uknown int 99 call: %d\", index);\r\n    }", "", "callHostFunction(NULL, common_int99, false, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "intIb", "IntIb", "cpu->thread->signalIllegalInstruction(5);// 5=ILL_PRVOPC  // :TODO: just a guess\r\n    NEXT_DONE();", fos32, fosOps_h, fosOps_cpp, "cpu->thread->signalIllegalInstruction(5);// 5=ILL_PRVOPC  // :TODO: just a guess", "", "callHostFunction(NULL, common_intIb, false, false, false, 1, 0, DYN_PARAM_CPU);\r\n    blockDone();", false, false);

        inst(fos, fos_init, "xlat", "Xlat", "if (op->ea16) {\r\n        AL = readb(cpu->seg[op->base].address + (U16)(BX + AL));\r\n    } else {\r\n        AL = readb(cpu->seg[op->base].address + EBX + AL);\r\n    }\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(reg[0].u8), DYN_8bit);\r\n    if (op->ea16) {\r\n        movToRegFromCpu(DYN_DEST, CPU_OFFSET_OF(reg[3].u16), DYN_16bit);\r\n        movToRegFromReg(DYN_SRC, DYN_16bit, DYN_SRC, DYN_8bit);\r\n        instRegReg('+', DYN_DEST, DYN_SRC, DYN_16bit);\r\n        movToRegFromReg(DYN_DEST, DYN_32bit, DYN_DEST, DYN_16bit);\r\n    } else {\r\n        movToRegFromCpu(DYN_DEST, CPU_OFFSET_OF(reg[3].u32), DYN_32bit);\r\n        movToRegFromReg(DYN_SRC, DYN_32bit, DYN_SRC, DYN_8bit);\r\n        instRegReg('+', DYN_DEST, DYN_SRC, DYN_32bit);\r\n    }\r\n    movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(seg[op->base].address), DYN_32bit);\r\n    instRegReg('+', DYN_DEST, DYN_SRC, DYN_32bit);\r\n    movFromMem(DYN_8bit, DYN_DEST);\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[0].u8), DYN_READ_RESULT, DYN_8bit);", true, false);
        inst(fos, fos_init, "hlt", "Hlt", "kpanic(\"Hlt\");", fos32, fosOps_h, fosOps_cpp, "", "", "kpanic(\"Dyn:Hlt\");", false, false);

        inst(fos, fos_init, "cmc", "Cmc", "cpu->fillFlags();\r\n    cpu->setCF(!cpu->getCF());\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "cpu->fillFlags();\r\n    cpu->setCF(!cpu->getCF());", "", "callHostFunction(NULL, common_cmc, true, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "clc", "Clc", "cpu->fillFlags();\r\n    cpu->removeCF();\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "cpu->fillFlags();\r\n    cpu->removeCF();", "", "callHostFunction(NULL, common_clc, true, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "stc", "Stc", "cpu->fillFlags();\r\n    cpu->addCF();\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "cpu->fillFlags();\r\n    cpu->addCF();", "", "callHostFunction(NULL, common_stc, true, false, false, 1, 0, DYN_PARAM_CPU);", true, false);

        inst(fos, fos_init, "cli", "Cli", "cpu->removeFlag(IF);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "cpu->removeFlag(IF);", "", "callHostFunction(NULL, common_cli, true, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "sti", "Sti", "cpu->addFlag(IF);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "cpu->addFlag(IF);", "", "callHostFunction(NULL, common_sti, true, false, false, 1, 0, DYN_PARAM_CPU);", true, false);

        inst(fos, fos_init, "cld", "Cld", "cpu->removeFlag(DF);\r\n    cpu->df=1;\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "cpu->removeFlag(DF);\r\n    cpu->df=1;", "", "callHostFunction(NULL, common_cld, true, false, false, 1, 0, DYN_PARAM_CPU);", true, false);
        inst(fos, fos_init, "std", "Std", "cpu->addFlag(DF);\r\n    cpu->df=-1;\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "cpu->addFlag(DF);\r\n    cpu->df=-1;", "", "callHostFunction(NULL, common_std, true, false, false, 1, 0, DYN_PARAM_CPU);", true, false);

        inst(fos, fos_init, "rdtsc", "Rdtsc", "U64 t = cpu->instructionCount+op->imm;\r\n    EAX = (U32)t;\r\n    EDX = (U32)(t >> 32);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, common_rdtsc, true, false, false, 2, 0, DYN_PARAM_CPU, op->imm, DYN_PARAM_CONST_32);", true, false);
        inst(fos, fos_init, "cpuid", "CPUID", "cpu->cpuid();\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "cpu->cpuid();", "", "callHostFunction(NULL, common_cpuid, true, false, false, 1, 0, DYN_PARAM_CPU);", true, false);

        inst(fos, fos_init, "enter16", "Enter16", "cpu->enter(0, op->imm, op->reg);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, common_enter, true, false, false, 4, 0, DYN_PARAM_CPU, 0, DYN_PARAM_CONST_32, op->imm, DYN_PARAM_CONST_32, op->reg, DYN_PARAM_CONST_32);", true, false);
        inst(fos, fos_init, "enter32", "Enter32", "cpu->enter(1, op->imm, op->reg);\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "callHostFunction(NULL, common_enter, true, false, false, 4, 0, DYN_PARAM_CPU, 1, DYN_PARAM_CONST_32, op->imm, DYN_PARAM_CONST_32, op->reg, DYN_PARAM_CONST_32);", true, false);

        inst(fos, fos_init, "leave16", "Leave16", "SP = BP;\r\n    BP = cpu->pop16();\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "movToCpuFromCpu(CPU_OFFSET_OF(reg[4].u16), CPU_OFFSET_OF(reg[5].u16), DYN_16bit, DYN_SRC);\r\n    callHostFunction(NULL, common_pop16, true, false, false, 1, 0, DYN_PARAM_CPU);\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[5].u16), DYN_CALL_RESULT, DYN_16bit);", true, false);
        inst(fos, fos_init, "leave32", "Leave32", "ESP = EBP;\r\n    EBP = cpu->pop32();\r\n    NEXT();", fos32, fosOps_h, fosOps_cpp, "", "", "movToCpuFromCpu(CPU_OFFSET_OF(reg[4].u32), CPU_OFFSET_OF(reg[5].u32), DYN_32bit, DYN_SRC);\r\n    callHostFunction(NULL, common_pop32, true, false, false, 1, 0, DYN_PARAM_CPU);\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[5].u32), DYN_CALL_RESULT, DYN_32bit);", true, false);
        ...
        inst(fos, fos_init, "loopnz", "LoopNZ", "if (op->ea16){\r\n        CX--;\r\n        if (CX!=0 && !cpu->getZF()) {\r\n            cpu->eip.u32+=op->imm;\r\n            NEXT_BRANCH1();\r\n        } else {\r\n            NEXT_BRANCH2();\r\n        }\r\n    } else {\r\n        ECX--;\r\n        if (ECX!=0 && !cpu->getZF()) {\r\n            cpu->eip.u32+=op->imm;\r\n            NEXT_BRANCH1();\r\n        } else {\r\n            NEXT_BRANCH2();\r\n        }\r\n    }", fos32, fosOps_h, fosOps_cpp, "CX--;\r\n    if (CX!=0 && !cpu->getZF()) {\r\n        cpu->eip.u32+=offset1;\r\n        NEXT_BRANCH1();\r\n    } else {\r\n        cpu->eip.u32+=offset2;\r\n        NEXT_BRANCH2();\r\n    }", ", U32 offset1, U32 offset2", "if (op->ea16) {\r\n        callHostFunction(NULL, common_loopnz, false, false, false, 3, 0, DYN_PARAM_CPU, op->imm+op->len, DYN_PARAM_CONST_32, op->len, DYN_PARAM_CONST_32);\r\n    } else {\r\n        callHostFunction(NULL, common_loopnz32, false, false, false, 3, 0, DYN_PARAM_CPU, op->imm+op->len, DYN_PARAM_CONST_32, op->len, DYN_PARAM_CONST_32);\r\n    }", false, false);
        ...
        inst(fos, fos_init, "loopz", "LoopZ", "if (op->ea16){\r\n        CX--;\r\n        if (CX!=0 && cpu->getZF()) {\r\n            cpu->eip.u32+=op->imm;\r\n            NEXT_BRANCH1();\r\n        } else {\r\n            NEXT_BRANCH2();\r\n        }\r\n    } else {\r\n        ECX--;\r\n        if (ECX!=0 && cpu->getZF()) {\r\n            cpu->eip.u32+=op->imm;\r\n            NEXT_BRANCH1();\r\n        } else {\r\n            NEXT_BRANCH2();\r\n        }\r\n    }", fos32, fosOps_h, fosOps_cpp, "CX--;\r\n    if (CX!=0 && !cpu->getZF()) {\r\n        cpu->eip.u32+=offset1;\r\n        NEXT_BRANCH1();\r\n    } else {\r\n        cpu->eip.u32+=offset2;\r\n        NEXT_BRANCH2();\r\n    }", ", U32 offset1, U32 offset2", "if (op->ea16) {\r\n        callHostFunction(NULL, common_loopz, false, false, false, 3, 0, DYN_PARAM_CPU, op->imm+op->len, DYN_PARAM_CONST_32, op->len, DYN_PARAM_CONST_32);\r\n    } else {\r\n        callHostFunction(NULL, common_loopz32, false, false, false, 3, 0, DYN_PARAM_CPU, op->imm+op->len, DYN_PARAM_CONST_32, op->len, DYN_PARAM_CONST_32);\r\n    }", false, false);
        ...
        inst(fos, fos_init, "loop", "Loop", "if (op->ea16){\r\n        CX--;\r\n        if (CX!=0) {\r\n            cpu->eip.u32+=op->imm;\r\n            NEXT_BRANCH1();\r\n        } else {\r\n            NEXT_BRANCH2();\r\n        }\r\n    } else {\r\n        ECX--;\r\n        if (ECX!=0) {\r\n            cpu->eip.u32+=op->imm;\r\n            NEXT_BRANCH1();\r\n        } else {\r\n            NEXT_BRANCH2();\r\n        }\r\n    }", fos32, fosOps_h, fosOps_cpp, "CX--;\r\n    if (CX!=0) {\r\n        cpu->eip.u32+=offset1;\r\n        NEXT_BRANCH1();\r\n    } else {\r\n        cpu->eip.u32+=offset2;\r\n        NEXT_BRANCH2();\r\n    }", ", U32 offset1, U32 offset2", "if (op->ea16) {\r\n        callHostFunction(NULL, common_loop, false, false, false, 3, 0, DYN_PARAM_CPU, op->imm+op->len, DYN_PARAM_CONST_32, op->len, DYN_PARAM_CONST_32);\r\n    } else {\r\n        callHostFunction(NULL, common_loop32, false, false, false, 3, 0, DYN_PARAM_CPU, op->imm+op->len, DYN_PARAM_CONST_32, op->len, DYN_PARAM_CONST_32);\r\n    }", false, false);
        ...
        inst(fos, fos_init, "jcxz", "Jcxz", "if ((op->ea16?CX:ECX)==0) {\r\n        cpu->eip.u32+=op->imm;\r\n        NEXT_BRANCH1();\r\n    } else {\r\n        NEXT_BRANCH2();\r\n    }", fos32, fosOps_h, fosOps_cpp, "if (CX==0) {\r\n        cpu->eip.u32+=offset1;\r\n        NEXT_BRANCH1();\r\n    } else {\r\n        cpu->eip.u32+=offset2;\r\n        NEXT_BRANCH2();\r\n    }", ", U32 offset1, U32 offset2", "if (op->ea16) {\r\n        callHostFunction(NULL, common_jcxz, false, false, false, 3, 0, DYN_PARAM_CPU, op->imm+op->len, DYN_PARAM_CONST_32, op->len, DYN_PARAM_CONST_32);\r\n    } else {\r\n        callHostFunction(NULL, common_jcxz32, false, false, false, 3, 0, DYN_PARAM_CPU, op->imm+op->len, DYN_PARAM_CONST_32, op->len, DYN_PARAM_CONST_32);\r\n    }", false, false);
    ...
    public void bound(FileOutputStream fos, FileOutputStream fos_init, String bits, String width, String offset, FileOutputStream fos32, FileOutputStream fosOps_h, FileOutputStream fosOps_cpp) throws IOException {
        ...
        out(fos32, "    callHostFunction(blockDone, common_bound"+bits+", true, false, true, 3, 0, DYN_PARAM_CPU, op->reg, DYN_PARAM_CONST_32, DYN_ADDRESS, DYN_PARAM_REG_32);");
    ...
    public void loadSeg(FileOutputStream fos, FileOutputStream fos_init, FileOutputStream fos32, String name, String ename, String bits, String width, String offset) throws IOException {
        ...
        out(fos32, "    callHostFunction(blockDone, common_setSegment, true, false, true, 3, 0, DYN_PARAM_CPU, op->imm, DYN_PARAM_CONST_32,  DYN_READ_RESULT, DYN_PARAM_REG_16);");
        out(fos32, "    movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u"+bits+"), DYN_DEST, DYN_"+bits+"bit);");
        out(fos32, "    INCREMENT_EIP(op->len);");
        out(fos32, "}");
    }
}
