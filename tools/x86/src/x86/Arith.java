...
public class Arith extends Base {
    ...
    public void generateAll(FileOutputStream fos, FileOutputStream fos_init, FileOutputStream fosOps_cpp, FileOutputStream fosOps_h, FileOutputStream fos32) throws IOException {
        ...
        dmul16(fos, fos_init, "dimulr16e16", "DimulR16E16", "readw(eaa(cpu, op))", "cpu->reg[op->reg].u16", fos32, "calculateEaa(op, DYN_ADDRESS); movFromMem(DYN_16bit, DYN_ADDRESS);", "DYN_READ_RESULT", "DYN_PARAM_REG_16", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit);", "DYN_SRC", "DYN_PARAM_REG_16");
        ...
        dmul32(fos, fos_init, "dimulr32e32", "DimulR32E32", "readd(eaa(cpu, op))", "cpu->reg[op->reg].u32", fos32, "calculateEaa(op, DYN_ADDRESS); movFromMem(DYN_32bit, DYN_ADDRESS);", "DYN_READ_RESULT", "DYN_PARAM_REG_32", "movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(reg[op->reg].u32), DYN_32bit);", "DYN_SRC", "DYN_PARAM_REG_32");
    }
    ...
    public void arith32(FileOutputStream fos, FileOutputStream fos_init, FileOutputStream fos32, String name, String op, boolean cf, boolean result, String bits, boolean flags, boolean iform, boolean hasRE)  throws IOException {
        ...
        if (hasRE) {
            if (result) {
                dyn = "if (!op->needsToSetFlags()) {\r\n        calculateEaa(op, DYN_ADDRESS);\r\n        "+(cf?"movToRegFromCpu(DYN_DEST, CPU_OFFSET_OF(reg[op->reg].u32), DYN_32bit);\r\n        instRegReg('" + op + "', DYN_DEST, DYN_CALL_RESULT, DYN_32bit);\r\n        movFromMem(DYN_32bit, DYN_ADDRESS);\r\n        instRegReg('" + op + "', DYN_DEST, DYN_READ_RESULT, DYN_32bit);\r\n        movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u32), DYN_DEST, DYN_32bit);":"movFromMem(DYN_32bit, DYN_ADDRESS);\r\n        instCPUReg('" + op + "', CPU_OFFSET_OF(reg[op->reg].u32), DYN_READ_RESULT, DYN_32bit);")+"\r\n    } else {\r\n        calculateEaa(op, DYN_ADDRESS);\r\n        movToCpuFromMem(CPU_OFFSET_OF(src.u32), DYN_32bit, DYN_ADDRESS);\r\n        movToCpuFromCpu(CPU_OFFSET_OF(dst.u32), CPU_OFFSET_OF(reg[op->reg].u32), DYN_32bit, DYN_DEST);\r\n        instRegReg(\'" + op + "\', DYN_DEST, DYN_READ_RESULT, DYN_32bit);\r\n"+(cf?"        movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(oldCF), DYN_32bit);\r\n        instRegReg(\'" + op + "\', DYN_DEST, DYN_SRC, DYN_32bit);\r\n":"")+"        movToCpuFromReg(CPU_OFFSET_OF(result.u32), DYN_DEST, DYN_32bit);\r\n        movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u32), DYN_DEST, DYN_32bit);"+(flags?"\r\n        movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"")+"\r\n    }";
            } else {
        ...
        if (iform) {
            if (result) {
                dyn = "if (!op->needsToSetFlags()) {\r\n        "+(cf?"instRegImm('+', DYN_CALL_RESULT, DYN_32bit, op->imm);\r\n        instCPUReg('"+op+"', CPU_OFFSET_OF(reg[op->reg].u32), DYN_CALL_RESULT, DYN_32bit);":"instCPUImm('" + op + "', CPU_OFFSET_OF(reg[op->reg].u32), DYN_32bit, op->imm);")+"\r\n    } else {\r\n        movToCpu(CPU_OFFSET_OF(src.u32), DYN_32bit, op->imm);\r\n        movToCpuFromCpu(CPU_OFFSET_OF(dst.u32), CPU_OFFSET_OF(reg[op->reg].u32), DYN_32bit, DYN_DEST);\r\n        instRegImm('" + op + "', DYN_DEST, DYN_32bit, op->imm);\r\n"+(cf?"        instRegReg(\'" + op + "\', DYN_DEST, DYN_CALL_RESULT, DYN_32bit);\r\n":"")+"        movToCpuFromReg(CPU_OFFSET_OF(result.u32), DYN_DEST, DYN_32bit);\r\n        movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u32), DYN_DEST, DYN_32bit);"+(flags?"\r\n        movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"")+"\r\n    }";
            } else {
                dyn = "movToCpu(CPU_OFFSET_OF(src.u32), DYN_32bit, op->imm);\r\n    movToCpuFromCpu(CPU_OFFSET_OF(dst.u32), CPU_OFFSET_OF(reg[op->reg].u32), DYN_32bit, DYN_DEST);\r\n    instRegImm(\'" + op + "\', DYN_DEST, DYN_32bit, op->imm);\r\n"+(cf?"    instRegReg(\'" + op + "\', DYN_DEST, DYN_CALL_RESULT, DYN_32bit);\r\n":"")+"    movToCpuFromReg(CPU_OFFSET_OF(result.u32), DYN_DEST, DYN_32bit);"+(result?"\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u32), DYN_DEST, DYN_32bit);":"")+(flags?"\r\n    movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"");
            }
    ...
    public void arith16(FileOutputStream fos, FileOutputStream fos_init, FileOutputStream fos32, String name, String op, boolean cf, boolean result, String bits, boolean flags, boolean iform, boolean hasRE)  throws IOException {
        ...
        String dyn;
        if (result) {
            dyn = "if (!op->needsToSetFlags()) {\r\n        movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(reg[op->rm].u16), DYN_16bit);\r\n        "+(cf?"movToRegFromReg(DYN_CALL_RESULT, DYN_16bit, DYN_CALL_RESULT, DYN_32bit);\r\n        instRegReg('+', DYN_SRC, DYN_CALL_RESULT, DYN_16bit);\r\n        ":"")+"instCPUReg('" + op + "', CPU_OFFSET_OF(reg[op->reg].u16), DYN_SRC, DYN_16bit);\r\n    } else {\r\n        movToCpuFromCpu(CPU_OFFSET_OF(src.u16), CPU_OFFSET_OF(reg[op->rm].u16), DYN_16bit, DYN_SRC);\r\n        movToCpuFromCpu(CPU_OFFSET_OF(dst.u16), CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit, DYN_DEST);\r\n        instRegReg(\'" + op + "\', DYN_DEST, DYN_SRC, DYN_16bit);\r\n" + (cf ? "        movToRegFromReg(DYN_CALL_RESULT, DYN_16bit, DYN_CALL_RESULT, DYN_32bit);\r\n        instRegReg(\'" + op + "\', DYN_DEST, DYN_CALL_RESULT, DYN_16bit);\r\n" : "") + "        movToCpuFromReg(CPU_OFFSET_OF(result.u16), DYN_DEST, DYN_16bit);" + (result ? "\r\n        movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u16), DYN_DEST, DYN_16bit);" : "")+(flags?"\r\n        movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"")+"\r\n    }";
        } else {
            dyn = "movToCpuFromCpu(CPU_OFFSET_OF(src.u16), CPU_OFFSET_OF(reg[op->rm].u16), DYN_16bit, DYN_SRC);\r\n    movToCpuFromCpu(CPU_OFFSET_OF(dst.u16), CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit, DYN_DEST);\r\n    instRegReg(\'" + op + "\', DYN_DEST, DYN_SRC, DYN_16bit);\r\n" + (cf ? "    movToRegFromReg(DYN_CALL_RESULT, DYN_16bit, DYN_CALL_RESULT, DYN_32bit);\r\n    instRegReg(\'" + op + "\', DYN_DEST, DYN_CALL_RESULT, DYN_16bit);\r\n" : "") + "    movToCpuFromReg(CPU_OFFSET_OF(result.u16), DYN_DEST, DYN_16bit);" + (result ? "\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u16), DYN_DEST, DYN_16bit);" : "")+(flags?"\r\n    movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"");
        }
        ...
        if (hasRE) {
            if (result) {
                dyn = "if (!op->needsToSetFlags()) {\r\n        calculateEaa(op, DYN_ADDRESS);\r\n        "+(cf?"movToRegFromCpu(DYN_DEST, CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit);\r\n        movToRegFromReg(DYN_CALL_RESULT, DYN_16bit, DYN_CALL_RESULT, DYN_32bit);\r\n        instRegReg('" + op + "', DYN_DEST, DYN_CALL_RESULT, DYN_16bit);\r\n        movFromMem(DYN_16bit, DYN_ADDRESS);\r\n        instRegReg('" + op + "', DYN_DEST, DYN_READ_RESULT, DYN_16bit);\r\n        movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u16), DYN_DEST, DYN_16bit);":"movFromMem(DYN_16bit, DYN_ADDRESS);\r\n        instCPUReg('" + op + "', CPU_OFFSET_OF(reg[op->reg].u16), DYN_READ_RESULT, DYN_16bit);")+"\r\n    } else {\r\n        calculateEaa(op, DYN_ADDRESS);\r\n        movToCpuFromMem(CPU_OFFSET_OF(src.u16), DYN_16bit, DYN_ADDRESS);\r\n        movToCpuFromCpu(CPU_OFFSET_OF(dst.u16), CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit, DYN_DEST);\r\n        instRegReg(\'" + op + "\', DYN_DEST, DYN_READ_RESULT, DYN_16bit);\r\n" + (cf ? "        movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(oldCF), DYN_32bit);\r\n        movToRegFromReg(DYN_SRC, DYN_16bit, DYN_SRC, DYN_32bit);\r\n        instRegReg(\'" + op + "\', DYN_DEST, DYN_SRC, DYN_16bit);\r\n" : "") + "        movToCpuFromReg(CPU_OFFSET_OF(result.u16), DYN_DEST, DYN_16bit);\r\n        movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u16), DYN_DEST, DYN_16bit);"+(flags?"\r\n        movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"")+"\r\n    }";
            } else {
                dyn = "calculateEaa(op, DYN_ADDRESS);\r\n    movToCpuFromMem(CPU_OFFSET_OF(src.u16), DYN_16bit, DYN_ADDRESS);\r\n    movToCpuFromCpu(CPU_OFFSET_OF(dst.u16), CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit, DYN_DEST);\r\n    instRegReg(\'" + op + "\', DYN_DEST, DYN_READ_RESULT, DYN_16bit);\r\n" + (cf ? "    movToRegFromCpu(DYN_SRC, CPU_OFFSET_OF(oldCF), DYN_32bit);\r\n    movToRegFromReg(DYN_SRC, DYN_16bit, DYN_SRC, DYN_32bit);\r\n    instRegReg(\'" + op + "\', DYN_DEST, DYN_SRC, DYN_16bit);\r\n" : "") + "    movToCpuFromReg(CPU_OFFSET_OF(result.u16), DYN_DEST, DYN_16bit);" + (result ? "\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u16), DYN_DEST, DYN_16bit);" : "")+(flags?"\r\n    movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"");
            }
        ...
        if (iform) {
            if (result) {
                dyn = "if (!op->needsToSetFlags()) {\r\n        "+(cf?"movToRegFromReg(DYN_CALL_RESULT, DYN_16bit, DYN_CALL_RESULT, DYN_32bit);\r\n        instRegImm('+', DYN_CALL_RESULT, DYN_16bit, op->imm);\r\n        instCPUReg('"+op+"', CPU_OFFSET_OF(reg[op->reg].u16), DYN_CALL_RESULT, DYN_16bit);":"instCPUImm('" + op + "', CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit, op->imm);")+"\r\n    } else {\r\n        movToCpu(CPU_OFFSET_OF(src.u16), DYN_16bit, op->imm);\r\n        movToCpuFromCpu(CPU_OFFSET_OF(dst.u16), CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit, DYN_DEST);\r\n        instRegImm(\'" + op + "\', DYN_DEST, DYN_16bit, op->imm);\r\n" + (cf ? "        movToRegFromReg(DYN_CALL_RESULT, DYN_16bit, DYN_CALL_RESULT, DYN_32bit);\r\n        instRegReg(\'" + op + "\', DYN_DEST, DYN_CALL_RESULT, DYN_16bit);\r\n" : "") + "        movToCpuFromReg(CPU_OFFSET_OF(result.u16), DYN_DEST, DYN_16bit);\r\n        movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u16), DYN_DEST, DYN_16bit);"+(flags?"\r\n        movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"")+"\r\n    }";
            } else {
                dyn = "movToCpu(CPU_OFFSET_OF(src.u16), DYN_16bit, op->imm);\r\n    movToCpuFromCpu(CPU_OFFSET_OF(dst.u16), CPU_OFFSET_OF(reg[op->reg].u16), DYN_16bit, DYN_DEST);\r\n    instRegImm(\'" + op + "\', DYN_DEST, DYN_16bit, op->imm);\r\n" + (cf ? "    movToRegFromReg(DYN_CALL_RESULT, DYN_16bit, DYN_CALL_RESULT, DYN_32bit);\r\n    instRegReg(\'" + op + "\', DYN_DEST, DYN_CALL_RESULT, DYN_16bit);\r\n" : "") + "    movToCpuFromReg(CPU_OFFSET_OF(result.u16), DYN_DEST, DYN_16bit);" + (result ? "\r\n    movToCpuFromReg(CPU_OFFSET_OF(reg[op->reg].u16), DYN_DEST, DYN_16bit);" : "")+(flags?"\r\n    movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"");
            }
    ...
    public void arith8(FileOutputStream fos, FileOutputStream fos_init, FileOutputStream fos32, String name, String op, boolean cf, boolean result, String bits, boolean flags, boolean iform, boolean hasRE)  throws IOException {
        ...
        if (iform) {
            if (result) {
                ...
                dyn = "movToCpu(CPU_OFFSET_OF(src.u8), DYN_8bit, op->imm);\r\n    movToCpuFromCpu(CPU_OFFSET_OF(dst.u8), OFFSET_REG8(op->reg), DYN_8bit, DYN_DEST);\r\n    instRegImm('" + op + "', DYN_DEST, DYN_8bit, op->imm);\r\n" + (cf ? "    movToRegFromReg(DYN_CALL_RESULT, DYN_8bit, DYN_CALL_RESULT, DYN_32bit);\r\n    instRegReg(\'" + op + "\', DYN_DEST, DYN_CALL_RESULT, DYN_8bit);\r\n" : "") + "    movToCpuFromReg(CPU_OFFSET_OF(result.u8), DYN_DEST, DYN_8bit);" + (result ? "\r\n    movToCpuFromReg(OFFSET_REG8(op->reg), DYN_DEST, DYN_8bit);" : "")+(flags?"\r\n    movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"")+(flags?"\r\n    movToCpu(CPU_OFFSET_OF(lazyFlags), Dyn_PtrSize, (DYN_PTR_SIZE)" + "FLAGS_"+name.toUpperCase()+bits + ");":"");
            }
