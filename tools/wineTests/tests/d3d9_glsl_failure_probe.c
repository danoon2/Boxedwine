/* Exercise generated GLSL compile/link failure isolation and later valid draws.
 * The --compile-fault / --link-fault oracle is used only with the separate test-only Wine injection.
...
static const struct phase phases[] = {
    ...
    {"combined-variants", D3DTSS_TCI_CAMERASPACEPOSITION, D3DTOP_SUBTRACT, 1},
    {"after-combined", D3DTSS_TCI_PASSTHRU, D3DTOP_SELECTARG1, 0},
};
...
static int check(int ok, unsigned line, const char *message)
{
    ...
    {
        ++failures;
        printf("glsl_failure_probe.c:%u: Test failed: %s: %s.\n", line, current, message);
    }
    return ok;
}
        ...
        IDirect3DSurface9 *readback, DWORD expected)
{
    ...
    for (i = 0; i < sizeof(positions)/sizeof(positions[0]); ++i)
    {
        ...
        printf("GLSL_FAILURE_PIXEL %s index=%u color=%08lx expected=%08lx\n",
...
static int run_cycle(IDirect3D9 *d3d, HWND window, unsigned cycle, int fault_mode)
{
    ...
    printf("GLSL_FAILURE_CYCLE cycle=%u\n", cycle);
    ...
    *(DWORD *)lock.pBits = 0xff8040c0;
    ...
    {
        int blocked = phases[i].blocked && (fault_mode != 2 || strcmp(phases[i].name,"combined-variants"));
        DWORD expected = fault_mode && blocked ? 0xff123456
        ...
        printf("GLSL_FAILURE_CASE %s blocked=%u fault=%u\n",current,blocked,fault_mode);
        REQUIRE(SetEnvironmentVariableA("BW_TEST_GLSL_PHASE",
                !strcmp(phases[i].name,"vertex-failure") ? "vertex" :
                !strcmp(phases[i].name,"pixel-failure") ? "pixel" : "none"),"set fault phase");
    ...
    if (device)
    {
        ...
        printf("GLSL_FAILURE_RELEASE cycle=%u refs=%lu\n",cycle,(unsigned long)refs);
...
int main(int argc, char **argv)
{
    ...
    unsigned cycle;
    int fault_mode = argc > 1 && !strcmp(argv[1],"--compile-fault") ? 1
            : argc > 1 && !strcmp(argv[1],"--link-fault") ? 2 : 0;
    setvbuf(stdout,NULL,_IONBF,0);
    printf("GLSL_FAILURE_BEGIN fault=%u\n",fault_mode);
    d3d = Direct3DCreate9(D3D_SDK_VERSION);
    REQUIRE(d3d,"create D3D9");
    REQUIRE(GetModuleFileNameA(GetModuleHandleA("d3d9.dll"),module,sizeof(module)),"D3D9 module path");
    printf("GLSL_FAILURE_MODULE d3d9.dll %s\n",module);
    window = CreateWindowA("static","GLSL shader failure",WS_OVERLAPPEDWINDOW,
            0,0,64,64,NULL,NULL,NULL,NULL);
    REQUIRE(window,"create window");
    for (cycle=0;cycle<3;++cycle) CHECK(run_cycle(d3d,window,cycle,fault_mode),"cycle completes");
    REQUIRE(completed == 27,"all phases complete");
done:
    if (d3d) IDirect3D9_Release(d3d);
    if (window) DestroyWindow(window);
    printf("GLSL_FAILURE_COVERAGE completed=%u\n",completed);
    printf("0000:glslfailure: %u tests executed (0 marked as todo, %u failures), 0 skipped.\n",tests,failures);
    return failures ? 1 : 0;
}
