/* Regression: split framebuffer readback across guest scheduling yields. */
...
int main(void)
{
    ...
    wc.lpszClassName = "BWReadBufferOwnershipProbe";
    check(RegisterClassA(&wc) != 0, "register window class");
    window = CreateWindowA(wc.lpszClassName, "Framebuffer read-buffer ownership regression", WS_OVERLAPPEDWINDOW | WS_VISIBLE,
    ...
    check(gen && bind && attach && status && delete_frames, "required framebuffer entry points");
    if (!gen || !bind || !attach || !status || !delete_frames) goto done;
    ...
    for (cycle = 0; cycle < 2; ++cycle)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 1, cycle ? 1 : 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    }
    ...
    for (cycle = 0; cycle < 128; ++cycle)
    {
        ...
        /* Binding restores this FBO's COLOR_ATTACHMENT0 read selection.
         * The last glReadBuffer call belongs to frames[1], where it is NONE. */
    ...
    if (!failures) printf("PASS framebuffer-owned read buffer across yields\n");
