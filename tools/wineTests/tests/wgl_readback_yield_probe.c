...
int main(void)
{
    ...
    wc.lpszClassName = "BWReadbackYieldProbe";
    check(RegisterClassA(&wc) != 0, "register window class");
    window = CreateWindowA(wc.lpszClassName, "Readback yield regression", WS_OVERLAPPEDWINDOW | WS_VISIBLE,
    ...
    if (!failures) printf("PASS split framebuffer readback across yields\n");
